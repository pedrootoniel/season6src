...
struct DANHHIEU_USER_DATA
{
	int		RankSlot1;
	int		RankTitle[20];
};

struct OFFLINE_MODE_DATA
{
	int UseBuff;
	int PickZen;
	int PickJewels;
...
struct OBJECTSTRUCT
{
	...
	BYTE HelperItems[12][16];
	int MuHelperItemsCount;
	int MuHelperItems[100];
#endif
	...
	int CrywolfMVPScore;
	DWORD LastCheckTick;
	DANHHIEU_USER_DATA Hornor;
	//Survivor Event
	int  SurvivorState; // 0 = none | 1 = signed to join | 2 = playing survivor | 3 = dead
	int  SurvivorKillCount;
	int  SurvivorDieCount;
	char Survivor_old_PK_Level;
	//Gift for noob
	BYTE TheGift;
	//
	DWORD RenameTime;
...
bool gObjIsAccountValidPassword(int aIndex,char* account, char* password);
...
extern int gObjTotalOffExp;
extern int gObjTotalOffTrade;
