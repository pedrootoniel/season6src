...
bool CItemManager::CheckItemMoveToTrade(LPOBJ lpObj,CItem* lpItem,BYTE TargetFlag) // OK
{
	...
	if(gServerInfo.LuckyItemTrade == 0)
	{
...
bool CItemManager::CheckItemMoveToVault(LPOBJ lpObj,CItem* lpItem,BYTE TargetFlag) // OK
{
	...
	if(gServerInfo.LuckyItemVault == 0)
	{
		if(lpItem->IsLuckyItem() != 0)
		{
			return 0;
...
bool CItemManager::CheckItemMoveToChaos(LPOBJ lpObj,CItem* lpItem,BYTE TargetFlag) // OK
{
	...
	if(gItemMove.CheckItemMoveAllowChaosBox(lpItem->m_Index) == 0)
	{		
		return 0;
	}
...
void CItemManager::InventoryItemSet(int aIndex,int slot,BYTE type) // OK
{
	...
	//g_QuestSystem.RunItem(&gObj[aIndex], NULL, slot);
}

void CItemManager::gObjTempInventoryItemBoxSet(BYTE * TempMap, int itempos, int xl, int yl, BYTE set_byte)
{
...
BYTE CItemManager::InventoryAddItem(int aIndex,CItem item,int slot) // OK
{
	...
	{
		if(this->CheckItemRequireClass(lpObj,item.m_Index) == 0 && lpObj->IsBot == 0)
		{
			return 0xFF;
		}
...
BYTE CItemManager::MoveItemToInventoryFromInventory(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if(lpObj->InventoryLock == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index,1, 0, 0, 0, 0, 0, gMessage.GetMessage(544));
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToInventoryFromWarehouse(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if(lpObj->WareLock == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index,1, 0, 0, 0, 0, 0, gMessage.GetMessage(545));
		return 0xFF;
	}
	...
	LogAdd(LOG_BLACK,"Player %s get item from vault to inventory (ItemIndex %d Serial: %d)",lpObj->Name,lpObj->Warehouse[SourceSlot].m_Index,lpObj->Warehouse[SourceSlot].m_Serial);
...
BYTE CItemManager::MoveItemToWarehouseFromInventory(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	LogAdd(LOG_BLACK,"Player %s put item from inventory to vault (ItemIndex %d Serial: %d)",lpObj->Name,lpObj->Inventory[SourceSlot].m_Index,lpObj->Inventory[SourceSlot].m_Serial);
	this->InventoryDelItem(lpObj->Index,SourceSlot);
...
BYTE CItemManager::MoveItemToChaosBoxFromInventory(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if(lpObj->ChaosMixLock == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index,1, 0, 0, 0, 0, 0, gMessage.GetMessage(546));
		return 0xFF;
	}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		if(gServerInfo.FruitOriginalWZ == 0)
		{
			if(lpMsg->type == 0)
			{
				if(gFruitSystem.UseFruitAddPoint(lpObj,((lpItem->m_Level<4)?(3-lpItem->m_Level):lpItem->m_Level)) != 0)
				{
					this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
					this->GCItemDeleteSend(aIndex,lpMsg->SourceSlot,1);
				}
			...
			{
				if(gFruitSystem.UseFruitSubPoint(lpObj,((lpItem->m_Level<4)?(3-lpItem->m_Level):lpItem->m_Level)) != 0)
				{
					this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
					this->GCItemDeleteSend(aIndex,lpMsg->SourceSlot,1);
				}
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->ShopLock == 1)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(543));
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	{
		if (lpObj->WCoinCaRecv < item.m_BuyWCoinC)
		{
	...
	{
		if (lpObj->WCoinPaRecv < item.m_BuyWCoinP)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(575));
			DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
	...
	{
		if (lpObj->GPointaRecv < item.m_BuyWCoinG)
		{
	...
	if(item.m_Index == GET_ITEM(13,4) || item.m_Index == GET_ITEM(13,5)) //Dark Horse,Dark Reaven
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(577), item.m_BuyWCoinC);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,item.m_BuyWCoinC,0,0);
		}
		else if (item.m_PriceType == WCOINP)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(578), item.m_BuyWCoinP);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,0,item.m_BuyWCoinP,0);
		}
		else if (item.m_PriceType == WCOING)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(579), item.m_BuyWCoinG);
			gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,0,0,item.m_BuyWCoinG);
		}
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
...
void CItemManager::CGItemBuyRecvCallback(LPOBJ lpObj,BYTE slot,DWORD null,DWORD WCoinC,DWORD WCoinP,DWORD GoblinPoint)
{
	//LPOBJ lpObj = &gObj[aIndex];
	...
	if(lpObj->ShopLock == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(543));
		DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	// Проверка есть ли необходимая сумма
	if (item.m_PriceType == ZEN)
	{
		if(((DWORD)lpObj->Money) < (item.m_BuyMoney+tax))
		{
	...
	LogAdd(LOG_BLACK, "[%s - %s] Buy [%d] slot %d", lpObj->Account,lpObj->Name,item.m_Index,slot);
}
...
void CItemManager::CGItemSellRecv(PMSG_ITEM_SELL_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpItem->m_IsExpiredItem == 0)
	{
		...
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,lpItem->m_SellWCoinC,0,0);
			//gCashShop.CGCashShopPointRecv(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(580), lpItem->m_SellWCoinC);
		}
		else if (lpItem->m_PriceType == WCOINP)
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,0,lpItem->m_SellWCoinP,0);
			//gCashShop.CGCashShopPointRecv(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(581), lpItem->m_SellWCoinP);
		}
		else if (lpItem->m_PriceType == WCOING)
		{
			gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,0,0,lpItem->m_SellWCoinG);
			//gCashShop.CGCashShopPointRecv(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(582), lpItem->m_SellWCoinG);
		}
	}
	else
	{
		...
		//int CreditsValue = gShopManager.GetPriceByItem(lpItem);
		//if (CreditsValue > 0)
		//{
		//	lpObj->BankEx.Credits += CreditsValue;
		//	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "You got %d %s! Balance: %d", CreditsValue,gMessage.GetMessage(527), lpObj->BankEx.Credits);
