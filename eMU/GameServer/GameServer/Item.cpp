...
{
	if (( this->m_Index >= ITEMGET(12,36) && this->m_Index <= ITEMGET(12,40) ) || (this->m_Index == ITEMGET(12,43)) || (this->m_Index == ITEMGET(12,50)) // Third Wings Prevent Wings Level 2 Attributes
#if (CRYSTAL_EDITION == 1)
			|| (this->m_Index >= ITEMGET(12,200) && this->m_Index <= ITEMGET(12,254))
#endif
		)
	{
		return 0;
...
LPSTR CItem::GetName()
{
	return &gItemManager.m_ItemInfo[this->m_Index].Name[0];
}

BOOL CItem::GetSize(int & w, int & h)
{
	w=gItemManager.m_ItemInfo[this->m_Index].Width  ;
	h=gItemManager.m_ItemInfo[this->m_Index].Height  ;
	return 1;
}
...
void CItem::Value() // OK
{
	...
	if(gItemValue.GetItemValue(this, &type, &BuyPrice, &SellPrice) != 0)
	{
		this->m_PriceType = type;
		if (type == ZEN) // Zen
		{
			this->m_BuyMoney = BuyPrice;
			...
			if (SellPrice < 0)
			{
				this->m_SellMoney = BuyPrice/3;
			}
			else
			{
				this->m_SellMoney = SellPrice;
			}
		...
		{
			this->m_BuyWCoinC = BuyPrice;

			this->m_SellWCoinC = SellPrice;
		}
		else if (type == WCOINP)
		{
			this->m_BuyWCoinP = BuyPrice;

			this->m_SellWCoinP = SellPrice;
		}
		else if (type == WCOING)
		{
			this->m_BuyWCoinG = BuyPrice;

			this->m_SellWCoinG = SellPrice;
		}
	}
