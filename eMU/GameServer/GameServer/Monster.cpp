...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	g_ShopPointEx.AddMonsterBonus(lpTarget->Index, lpObj->Index);
...
void gObjMonsterDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Map == MAP_KALIMA6)
	{
		if(lpObj->Class == 338)
		{
			if(OBJECT_RANGE(lpTarget->PartyNumber) == 0)
			{
				gObjMoveGate(lpTarget->Index,116);
			}
			else
			{
				for(int n=0;n < MAX_PARTY_USER;n++)
				{
					if(OBJECT_RANGE(gParty.m_PartyInfo[lpTarget->PartyNumber].Index[n]) != 0)
					{
						gObj[gParty.m_PartyInfo[lpTarget->PartyNumber].Index[n]].Interface.use = 0;
						gObj[gParty.m_PartyInfo[lpTarget->PartyNumber].Index[n]].Interface.type = INTERFACE_NONE;
						gObj[gParty.m_PartyInfo[lpTarget->PartyNumber].Index[n]].Interface.state = 0;
						gObjMoveGate(gParty.m_PartyInfo[lpTarget->PartyNumber].Index[n],116);
					}
				}
			}
		}
	...
	//g_QuestSystem.RunHunt(lpObj,lpTarget);
...
{
	if(lpObj->IsBot == 1 && lpObj->BotNumOwner <= 0)
	{
		return -1;
	}
	int n;
		...
		if ( lpObj->Attribute )
		{
			...
			if (lpObj->Class == 249 || lpObj->Class == 247 )
			{
				...
				if( !(rand()%3) )
				{
					if( lpObj->PKCount == 0 )
					{
						ChatSend(lpObj, gServerInfo.GuardMessage);
					}
				}
					...
					if(lpObj->IsBot >= 1)
					{
						switch(lpObj->IsBot)
						{
							case 2:		//Bot Alchemist
							{
								if ( !(rand()%70) )
								{
									ChatSend(lpObj, gMessage.GetMessage(630));
									GCActionSend(lpObj,147,lpObj->Index,0);
								}
							}
							break;
							case 3:		//Bot Buffer
							{
								if ( !(rand()%70) )
								{
									ChatSend(lpObj, gMessage.GetMessage(631));
									GCActionSend(lpObj,147,lpObj->Index,0);
								}
							}
							break;
							...
							case 5:		//Bot Trader
							{
								if ( !(rand()%70) )
								{
									ChatSend(lpObj, gMessage.GetMessage(632));
									GCActionSend(lpObj,147,lpObj->Index,0);
								}
							}
							break;
