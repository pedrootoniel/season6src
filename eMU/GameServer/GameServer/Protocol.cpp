...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
		...
		case 0x66:
			...
			break;
        case 0x78:
             GCPostItemProc((PMSG_POST_ITEM_RECV*)lpMsg,aIndex);
            break;
		case 0x79:
			gRolltheDice.StartRoll(lpObj,(ROLL_DICE_REQ*)lpMsg);
		break;
		case 0x81:
		...
		case 0x91:
			...
			break;
		case 0x95:
			gGoldenArcher.CGRegEventChipRecv((PMSG_REGEVENTCHIP*)lpMsg,aIndex);
			break;
		case 0x97:
			gGoldenArcher.GCUseEndEventChipRecv(aIndex);
			break;
		case 0x9A:
		...
		case 0xFB:
			...
			CGSNSDataRecv((PMSG_SNS_DATA_RECV*)lpMsg,aIndex);
				...
				{
				case 0x06:
					gHeadsTails.StartFlip(lpObj,(HEADS_TAILS_REQ*)lpMsg);
					break;
				case 0x07:
					gLuckyNumber.Start(lpObj,(LUCKYNUMBER_REQ*)lpMsg);
					break;
				case 0x08:
					//SVShop.RecvBuy(aIndex,(SDHP_BUYSVSHOP*)lpMsg);
					break;
				case 0x12:
					gDanhHieu.ActiveTitle(lpObj,(ACTIVE_TITLE_REQ*)lpMsg);
					break;
				case 0x13:
					gDanhHieu.BuyTitle(lpObj,(BUY_TITLE_REQ*)lpMsg);
					break;
				case 0x14:
					gDanhHieu.Deactive(lpObj);
					break;
				case 0x20:
				...
				case 0x27:
					...
					break;
				case 0x28:
					CGReqFileCrcRecv(aIndex,(PMSG_REQ_CRCDATA*)lpMsg);
					break;
				case 0x29:
		...
		case 0xFC:
			...
			break;
		case 0xFD:
			gNewsBoard.OpenMain(&gObj[aIndex]);
			break;
		case 0xFE:
			gNewsBoard.OpenItem(&gObj[aIndex], (NEWS_REQ_NEWS*)lpMsg);
			break;
        case 0xFF:
            switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
            {
				case 0x01:
					gAccSecurity.gObjAccountSecuritySave((PMSG_ACCOUNT_SECURITY_RECV*)lpMsg,aIndex);
					break;
				case 0x02:
					gAccSecurity.gObjAccountSecurityPW((PMSG_SECURITY_PW_RECV*)lpMsg,aIndex);
					break;
				case 0x03:
					gObjRenameCharacter((PMSG_RENAME_RECV*)lpMsg,lpObj);
					break;
				case 0x04:
					GCUpdateBalance(aIndex);
					break;
				case 0x05:
					gChangeClass.RecvChangeClass((CG_CHANGECLASS_RECV*)lpMsg, aIndex);
					break;
				case 0x06:
					gBuyAccountLevel.RecvBuyVip((CG_BUYVIP_RECV*)lpMsg, aIndex);
					break;
				case 0x0A:
				...
				case 0x17:
					...
					break;
				case 0x18:
					gBankEx.JewelRecv((PMSG_JEWELSBANK_RECV*)lpMsg,lpObj);
					break;
				case 0x19:
					gBankEx.JewelSend((PMSG_JEWELSBANK_RECV*)lpMsg,lpObj);
					break;
				case 0x20:
					gPersonalShop.CGPShopItemListRecv((PMSG_PSHOP_ITEM_LIST_RECV*)lpMsg,aIndex);
					break;
				case 0x21:
					gRanking.SendUser(lpObj);
					break;
				case 0x22:
					g_PersonalShopEx.Search(aIndex,(CG_PersonalPage*)lpMsg);
					break;
				case 0x23:
					gPersonalShop.CGPShopCloseRecv(aIndex);
					break;
				case 0x24:
					...
					break;
				case 0x25:
					gObjGiftCode((PMSG_GIFTCODE_SEND*)lpMsg,aIndex);
					break;
				case 0x26:
					gParty.TracePlayer(&gObj[aIndex],(BYTE)*(lpMsg+4));
					break;
				case 0x1C:
					gEventTimer.CG_RequestEvents((CG_EVENTTIMER_REQ*)lpMsg,aIndex);
					break;
				case 0xE5:
					CGGetWarehouseNumber((PMSG_CHANGEWAREHOUSE_RECV*)lpMsg, aIndex);
					break;
			}
			break;
	}
...
void CGCharacterDeleteRecv(PMSG_CHARACTER_DELETE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->DelCharLock == 1)
	{
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(653));
		pMsg.result = 3;
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
...
void GCNewHealthBarSend(LPOBJ lpObj) // OK
{
	...
	//if (gMapManager.GetMapHPBarEnable(lpObj->Map) == 0)
	//{
	//	return;
	//}
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpObj->VpPlayer[n].type != OBJECT_MONSTER && lpObj->VpPlayer[n].type != OBJECT_USER)
		{
			continue;
		}
		...
		//if (lpTarget->Type == OBJECT_USER) //&& gMapManager.GetMapGensBattle(lpTarget->Map) != 0)
		//{
		//	continue;
		//}
		...
		if (lpTarget->Type == OBJECT_USER)
		{
			...
			if (lpTarget->MaxShield+lpTarget->AddShield > 0)
			{
				info.SDrate = (BYTE)((lpTarget->Shield*100)/(lpTarget->MaxShield+lpTarget->AddShield));

				info.Shield = lpTarget->Shield;

				info.MaxShield = lpTarget->MaxShield + lpTarget->AddShield;

			}

			info.isSameGuild = (lpObj->GuildNumber>0?((lpObj->GuildNumber == lpTarget->GuildNumber)?true:false):false);

			info.RankTitle = lpTarget->Hornor.RankSlot1;

			/*
			info.HaveGuild = (lpTarget->GuildNumber>0?true:false);

			if(info.HaveGuild = true)
			{
				memcpy(info.GuildName,lpTarget->GuildName,sizeof(info.GuildName));
...
void TargetDataRequest(LPOBJ lpObj)
{
	...
	for(n = 0; n < MAX_VIEWPORT; n++)
	{
		if(lpObj->VpPlayer[n].state != VIEWPORT_SEND && lpObj->VpPlayer[n].state != VIEWPORT_WAIT)
		{
			tObjNum = lpObj->VpPlayer[n].index;
			...
			if (gObj[tObjNum].Type == OBJECT_MONSTER)
			{
				...
				DataSend(lpObj->Index, (BYTE*)&pMonMsg, sizeof(pMonMsg));
			}
			else
			{
				pMsg.aIndex = tObjNum;
				...
				pPlayerMsg.SurvivorDie = lpObj->SurvivorDieCount;
				pPlayerMsg.SurvivorKill = lpObj->SurvivorKillCount;	
				pPlayerMsg.RankTitle = gObj[tObjNum].Hornor.RankSlot1;
				...
				DataSend(lpObj->Index, (BYTE*)&pPlayerMsg, sizeof(pPlayerMsg));
			}
		}
	}
	#endif
...
void gDanhHieuDataSend(LPOBJ lpObj)
{
	// ----
	PMSG_HORNOR_UPD pMsg;
	pMsg.h.set(0xFB, 0x15, sizeof(pMsg));
	// ----
	pMsg.RankSlot		= lpObj->Hornor.RankSlot1;
	pMsg.RankTitle[0]	= lpObj->Hornor.RankTitle[0];
	pMsg.RankTitle[1]	= lpObj->Hornor.RankTitle[1];
	pMsg.RankTitle[2]	= lpObj->Hornor.RankTitle[2];
	pMsg.RankTitle[3]	= lpObj->Hornor.RankTitle[3];
	pMsg.RankTitle[4]	= lpObj->Hornor.RankTitle[4];
	pMsg.RankTitle[5]	= lpObj->Hornor.RankTitle[5];
	pMsg.RankTitle[6]	= lpObj->Hornor.RankTitle[6];
	pMsg.RankTitle[7]	= lpObj->Hornor.RankTitle[7];
	pMsg.RankTitle[8]	= lpObj->Hornor.RankTitle[8];
	pMsg.RankTitle[9]	= lpObj->Hornor.RankTitle[9];
	pMsg.RankTitle[10]	= lpObj->Hornor.RankTitle[10];
	pMsg.RankTitle[11]	= lpObj->Hornor.RankTitle[11];
	pMsg.RankTitle[12]	= lpObj->Hornor.RankTitle[12];
	pMsg.RankTitle[13]	= lpObj->Hornor.RankTitle[13];
	pMsg.RankTitle[14]	= lpObj->Hornor.RankTitle[14];
	pMsg.RankTitle[15]	= lpObj->Hornor.RankTitle[15];
	pMsg.RankTitle[16]	= lpObj->Hornor.RankTitle[16];
	pMsg.RankTitle[17]	= lpObj->Hornor.RankTitle[17];
	pMsg.RankTitle[18]	= lpObj->Hornor.RankTitle[18];
	pMsg.RankTitle[19]	= lpObj->Hornor.RankTitle[19];
	// CUSTOM EVENT INFO
	pMsg.SurvivorPlayer	= g_Survivor.PlayerCount;
	pMsg.IsSurvivor	=	lpObj->SurvivorState;
	// ----
	DataSend(lpObj->Index,(BYTE*)&pMsg, sizeof(pMsg));
}

void RolltheDiceData(int aIndex)
{
	...
	LPOBJ lpObj = &gObj[aIndex];
	DICE_UPDATEINFO pRequest;
	pRequest.header.set(0xFB,0x17,sizeof(pRequest));
	pRequest.Dice1		= gRolltheDice.Dice1;
	pRequest.Dice2		= gRolltheDice.Dice2;
	pRequest.TotalPoint		= gRolltheDice.TotalPoint;
	pRequest.LastProfit	= gRolltheDice.LastProfit;
	pRequest.WCoinC	=	gObj[aIndex].WCoinCaRecv;
	pRequest.Winner	=	gRolltheDice.Winner;
	DataSend(aIndex,(BYTE*)&pRequest,pRequest.header.size);
}

void HeadsTailsData(int aIndex)
{
	if( !OBJECT_RANGE(aIndex) )
	{
		return;
	...
	LPOBJ lpObj = &gObj[aIndex];
	HEADSTAILS_UPDATEINFO pMsg;
	pMsg.header.set(0xFB,0x16,sizeof(pMsg));
	pMsg.Winer		= gHeadsTails.winer;
	pMsg.WinerPrice = gHeadsTails.winerprice;
	pMsg.WCoinC = gObj[aIndex].WCoinCaRecv;
	DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
}

void LuckyNumberData(int aIndex)
{
	if( !OBJECT_RANGE(aIndex) )
	{
		return;
	...
	LPOBJ lpObj = &gObj[aIndex];
	LUCKYNUMBER_UPDATEINFO pMsg;
	pMsg.header.set(0xFB,0x18,sizeof(pMsg));
	//pMsg.WinnerPrice = gObj[aIndex].PlayerRank;
	pMsg.WinnerPrice = gLuckyNumber.WinPrice;
	pMsg.LuckyNumber = gLuckyNumber.LuckyNumber;
	pMsg.Number[0] = gLuckyNumber.Number[0];
	pMsg.Number[1] = gLuckyNumber.Number[1];
	pMsg.Number[2] = gLuckyNumber.Number[2];
	pMsg.Number[3] = gLuckyNumber.Number[3];
	pMsg.Number[4] = gLuckyNumber.Number[4];
	pMsg.TicketID = gLuckyNumber.TicketID;
	DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
}
...
void GCPostItemProc(PMSG_POST_ITEM_RECV* lpMsg, int aIndex ) //ok
{
	...
	{
		GDGlobalItemPostSend(gMapServerManager.GetMapServerGroup(),lpMsg->chatid,lpMsg->chatmsg,lpMsg->item_data);
	}
}
...
void ExtraCpanelSendInfo(int aIndex)
{
	...
	pInfo.Settings		=	gServerInfo.m_SettingsButton;
	pInfo.RolltheDice	=	gServerInfo.m_RolltheDiceButton;
	pInfo.HeadsTails	=	gServerInfo.m_HeadsTailButton;
	pInfo.LuckyNumber	=	gServerInfo.m_LuckyNumberButton;
	pInfo.SmithItem		=	gServerInfo.m_SmithItemButton;
	pInfo.TitleManager	=	gServerInfo.m_TitleManagerButton;
	pInfo.PartySearchSetting	=	gServerInfo.m_PartySearchSettingButton;
	pInfo.OffAttack	=	gServerInfo.m_OffAttackButton;
	pInfo.BuyVip	=	gServerInfo.m_BuyVipButton;
	pInfo.ChangeClass	=	gServerInfo.m_ChangeClassButton;
	pInfo.ResetPoint	=	gServerInfo.m_ResetPointCheckBox;
	pInfo.ResetPoint2	=	gServerInfo.m_ResetPointCheckBox2;
	pInfo.ResetMasterSkill	=	gServerInfo.m_ResetMasterSkillCheckBox;
	pInfo.ResetMasterSkill2	=	gServerInfo.m_ResetMasterSkillCheckBox2;
	pInfo.AccountSecurity	=	gServerInfo.m_AccountSecurityButton;
	pInfo.JewelsBank	=	gServerInfo.m_JewelsBankButton;
	pInfo.Rename	=	gServerInfo.m_RenameButton;
	pInfo.TopPlayer	=	gServerInfo.m_TopPlayerButton;
	pInfo.PShopCurrency	=	gServerInfo.m_PShopCurrencyButton;
	pInfo.PShopOffStore	=	gServerInfo.m_PShopOffStoreButton;
	pInfo.RPSMode	=	gServerInfo.m_RPSModeButton;
	pInfo.NewsBoard	=	gServerInfo.m_NewsBoardButton;
	pInfo.FruitMain = gServerInfo.m_FruitMainButton;
	pInfo.LuckySpinMain = gServerInfo.m_LuckySpinButton;
	pInfo.GiftCode = gServerInfo.m_GiftCodeButton;
	pInfo.PSBless = gServerInfo.m_BlessButton;
	pInfo.PSSoul = gServerInfo.m_SoulButton;
	pInfo.PSChaos = gServerInfo.m_ChaosButton;
	pInfo.PSWcoinC = gServerInfo.m_wCoinCButton;
	pInfo.PSWcoinP = gServerInfo.m_wCoinPButton;
	pInfo.PSWcoinG = gServerInfo.m_GoblinPointButton;
	pInfo.QuickAdd = gServerInfo.m_QuickAddButton;
	pInfo.ResetStat = gServerInfo.m_ResetStatButton;
	pInfo.EventTimer = gServerInfo.m_EventTimerButton;
	pInfo.CustomWare = gServerInfo.m_CustomWareButton;
	pInfo.PartyLeader = gServerInfo.m_PartyLeaderButton;
	pInfo.PartyTrace = gServerInfo.m_PartyTraceButton;
	pInfo.TitleBarSwitch	=	gServerInfo.m_TitleBarSwitch;
	pInfo.PlayerHPBarSwitch	=	gServerInfo.m_PlayerHPBarSwitch;
	pInfo.SDByPass = gServerInfo.m_BypassShield;
	...
	pInfo.MaxWarehouse = gServerInfo.m_CommandWareNumber[lpObj->AccountLevel];
	for (int i=0; i<MAX_ACCOUNT_LEVEL; i++)
	{
		pInfo.CustomResetPointPriceWC[i] = gServerInfo.ResetPointWC[i];
		pInfo.CustomResetPointPriceZen[i] = gServerInfo.ResetPointZEN[i];
		pInfo.CustomMasterSkillPriceWC[i] = gServerInfo.ResetMasterWC[i];
		pInfo.CustomMasterSkillPriceZen[i] = gServerInfo.ResetMasterZEN[i];
...
void CGResetStatsRecv(PMSG_RESET_STATS_RECV* lpMsg, int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];

	if (lpMsg->Type == 0)
	{
		if (lpObj->WCoinCaRecv < gServerInfo.ResetPointWC[lpObj->AccountLevel])
		{
		...
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,gServerInfo.ResetPointWC[lpObj->AccountLevel],0,0);
	...
	if (lpMsg->Type == 1)
	{
		if(lpObj->Money < gServerInfo.ResetPointZEN[lpObj->AccountLevel])
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(614));
			return;
		}
		...
		lpObj->Money -= gServerInfo.ResetPointZEN[lpObj->AccountLevel];
	...
	if(lpMsg->Type == 2)
	{
		if(lpObj->WCoinCaRecv < ((DWORD)gServerInfo.ResetMasterWC[lpObj->AccountLevel]))
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(574));
			return;
		}
		...
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,gServerInfo.ResetMasterWC[lpObj->AccountLevel],0,0);
	}

	if(lpMsg->Type == 3)
	{
		if(lpObj->Money < gServerInfo.ResetMasterZEN[lpObj->AccountLevel])
		{
			gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(614));
			return;
		}
		...
		lpObj->Money -= gServerInfo.ResetMasterZEN[lpObj->AccountLevel];
...
void GCUpdateBalance(int aIndex)
{
	...
	PMSG_BALANCE_UPDATE_SEND pMsg;
	pMsg.header.set(0xFB, 0x06, sizeof(pMsg));
	// ----
	pMsg.wCoinC = lpObj->WCoinCaRecv;
	pMsg.wCoinP = lpObj->WCoinPaRecv;
	pMsg.GoblinPoint = lpObj->GPointaRecv;
	// ----
	DataSend(aIndex, (LPBYTE)&pMsg, sizeof(pMsg));
}

void GCAccountSecuritySend(int aIndex)
{
	if (!gObjIsConnectedGP(aIndex))
	{
		return;
	...
	pMsg.header.set(0xFB, 0x01, sizeof(pMsg));
	// ----
	pMsg.TradeLock = lpObj->TradeLock;
	pMsg.InventoryLock = lpObj->InventoryLock;
	pMsg.WareLock = lpObj->WareLock;
	pMsg.ShopLock = lpObj->ShopLock;
	pMsg.PShopLock = lpObj->PShopLock;
	pMsg.ChaosMixLock = lpObj->ChaosMixLock;
	pMsg.DelCharLock = lpObj->DelCharLock;
	// ----
	DataSend(aIndex, (LPBYTE)&pMsg, sizeof(pMsg));
}

void GCCharacterPageSend(LPOBJ lpObj)
{
	PMSG_CHARACTERPAGE_SEND pMsg;

	pMsg.header.set(0xFB,0x11,sizeof(pMsg));
...
void gObjRenameCharacter(PMSG_RENAME_RECV* lpMsg, LPOBJ lpObj)
{
	if (lpObj->WCoinCaRecv < gServerInfo.ChangeNameWC[lpObj->AccountLevel])
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(519),gServerInfo.ChangeNameWC[lpObj->AccountLevel]);
		return;
	}
	gCommandManager.CommandRename(lpObj,lpMsg->Name);
}
...
void gObjGiftCode(PMSG_GIFTCODE_SEND* lpMsg, int aIndex)
{
	GiftCode_Sen(aIndex,lpMsg->ID);
}

void CGGetWarehouseNumber(PMSG_CHANGEWAREHOUSE_RECV* lpMsg, int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	lpObj->WarehouseNumber = lpMsg->Number;

	gLog.Output(LOG_COMMAND,"[CustomWarehouse][%s][%s] - Open Ware: %d)",lpObj->Account,lpObj->Name,lpMsg->Number);
