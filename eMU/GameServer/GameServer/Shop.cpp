...
void CShop::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			int	Socket = lpMemScript->GetAsNumber();
			...
			int SocketBonus = lpMemScript->GetAsNumber();

			this->InsertItem(ItemIndex,ItemLevel,ItemDurability,ItemOption1,ItemOption2,ItemOption3,ItemNewOption,AncOption,ItemOptionEx,Socket,Socket2,Socket3,Socket4,Socket5,SocketBonus,0);
		}
	}
	catch(...)
	{
...
void CShop::InsertItem(int ItemIndex,int ItemLevel,int ItemDurability,int ItemOption1,int ItemOption2,int ItemOption3,int ItemNewOption,int AncOption,int ItemOptionEx,int Socket,int Socket2,int Socket3,int Socket4,int Socket5,int SocketBonus,int ItemValue) // OK
{
	ITEM_INFO ItemInfo;
		...
		for(int x=0;x < 8;x++)
		{
			if(this->m_InventoryMap[((y*8)+x)] == 0xFF)
			{
				...
				if(slot != 0xFF)
				{
					...
					s_SocketOption[0] = Socket;
					s_SocketOption[1] = Socket2;
					s_SocketOption[2] = Socket3;
					s_SocketOption[3] = Socket4;
					s_SocketOption[4] = Socket5;		
					this->m_Item[slot].Convert(ItemIndex,ItemOption1,ItemOption2,ItemOption3,ItemNewOption,AncOption,0,ItemOptionEx,s_SocketOption,SocketBonus);
