...
void CDevilSquare::GiveUserRewardMoney(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_DS_USER;n++)
	{
		...
		if (lpLevel->User[n].Rank == 0)
		{
			...
			{
				//Reward
				gCashShop.GDCashShopAddPointSaveSend(lpLevel->User[n].Index,0,this->m_DevilSquareRewardValue[lpLevel->Level][0],this->m_DevilSquareRewardValue[lpLevel->Level][1],this->m_DevilSquareRewardValue[lpLevel->Level][2]);
