...
void CWarehouse::CGWarehouseClose2(int aIndex, int type) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if(lpObj->Interface.use == 0 || lpObj->Interface.type != INTERFACE_WAREHOUSE || lpObj->Interface.state == 0)
	{
		//LogAdd(LOG_RED, "Warehouse 0!");
		return;
	}

	if(lpObj->LoadWarehouse == 0)
	{
		//LogAdd(LOG_RED, "Warehouse 1!");
		return;
	}
	...
	// %d %d %d %d %d", lpObj->Interface.use, lpObj->Interface.type, lpObj->Interface.state, lpObj->LoadWarehouse, lpObj->WarehouseLock);
	//LogAdd(LOG_RED, "Warehouse closed! type [%d] (%d %d %d %d %d)", type, lpObj->Interface.use, lpObj->Interface.type, lpObj->Interface.state, lpObj->LoadWarehouse, lpObj->WarehouseLock);
}
...
void CWarehouse::CGWarehouseClose(int aIndex) // OK
{
	...
	lpObj->VaultTimeCount = gServerInfo.VaultOpenDelay;
