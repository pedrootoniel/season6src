...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	strcpy_s(this->m_DisplayerText[1],"SupremoTeam");
}
...
void CServerDisplayer::Init(HWND hWnd) // OK
{
	...
	gLog.AddLog(gServerInfo.m_WriteInstaGamesLog,"INSTAGAMES_LOG");

	gLog.AddLog(gServerInfo.m_WriteSmithItemLog,"SMITHITEM_LOG");

	gLog.AddLog(gServerInfo.m_WriteQuestLog,"CUSTOM_QUEST");

	gLog.AddLog(gServerInfo.m_WriteItemLog,"PLAYER_LOG_ITEM");
}
...
void CServerDisplayer::SetWindowName() // OK
{
	...
	wsprintf(buff,"[%s] %s (Player: %d/%d) (OffStore: %d) (OffAttack: %d) (Monster: %d/%d)",GAMESERVER_VERSION,gServerInfo.m_ServerName,gObjTotalUser,gServerInfo.m_ServerMaxUserNumber,gObjTotalOffTrade,gObjTotalOffExp,gObjTotalMonster,MAX_OBJECT_MONSTER);
