...
CInvasionManager::CInvasionManager() // OK
{
	...
	this->GoldDerconRegen = false;
	this->GoldDerconMap = -1;
	this->GoldDerconCount = 0;
	this->GGDRegen = false;
	this->GGDMap = -1;
	this->GGDCount = 0;
	this->RedDragonRegen = false;
	this->RedDragonMap = -1;
...
void CInvasionManager::SetMonster(INVASION_INFO* lpInfo,INVASION_RESPWAN_INFO* lpRespawnInfo,INVASION_MONSTER_INFO* lpMonsterInfo) // OK
{
	for(int n=0;n < gMonsterSetBase.m_count;n++)
	{
		...
		if (lpObj->Class == 79)
		{
			this->GoldDerconRegen = true;
			this->GoldDerconMap = lpObj->Map;
			this->GoldDerconCount++;
			//this->CheckGoldDercon(lpObj->Map);
		}

		if (lpObj->Class == 501)
		{
			this->GGDRegen = true;
			this->GGDMap = lpObj->Map;
			this->GGDCount++;
			//this->CheckGreatGoldDragon(lpObj->Map);
		}

		if (lpObj->Class == 42)
		{
			this->RedDragonRegen = true;
			this->RedDragonMap = lpObj->Map;
			this->RedDragonCount++;
			//this->CheckRedDragon(lpObj->Map);
		}
...
void CInvasionManager::CheckGoldDercon(int MapNumber)
{
	if ( this->GoldDerconRegen == false )
	{
		return;
	}

	if (this->GoldDerconMap != MapNumber)
	{
		return;
	}
...
void CInvasionManager::CheckGreatGoldDragon(int MapNumber)
{
	if ( this->GGDRegen == false )
	{
		return;
	}

	if (this->GGDMap != MapNumber)
	{
		return;
	}
...
void CInvasionManager::CheckRedDragon(int MapNumber)
{
	if ( this->RedDragonRegen == false )
	{
		return;
	}

	if (this->RedDragonMap != MapNumber)
	{
		return;
	}
	...
	if (this->RedDragonCount > 0)
	{
		GCMapEventStateSend(MapNumber, 1, 1);
	}
	else
	{
		GCMapEventStateSend(MapNumber, 0, 1);
	}
}