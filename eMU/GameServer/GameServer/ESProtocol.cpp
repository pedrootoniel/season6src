...
void GDGuildCreateRecv(SDHP_GUILDCREATE_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		if ( aIndex >= 0 && lpNode != NULL )
		{
			...
			#pragma message("PVS_STUDIO FIX 06");
			memcpy(gObj[aIndex].GuildName,szGuildName,sizeof(szGuildName));
...
void GDGuildMemberAddResult(SDHP_GUILDMEMBERADD_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		if ( HereUserIndex >= 0 && lpNode != NULL )
		{
			...
			#pragma message("PVS_STUDIO FIX 07");
			memcpy(gObj[HereUserIndex].GuildName,szGuildName,sizeof(szGuildName));
		...
		if ( gObj[n].Connected > OBJECT_LOGGED )
		{
			if ( gObj[n].Name[0] == lpMsg->MemberID[0] )
			{
				if ( strcmp(lpMsg->MemberID, gObj[n].Name ) == 0 )
				{
					...
					#endif
					#pragma message("PVS_STUDIO FIX 08");
					memcpy(gObj[n].GuildName,szGuildName,sizeof(szGuildName));
