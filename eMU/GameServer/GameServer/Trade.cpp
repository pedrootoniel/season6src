...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gBotBuffer.TradeOpen(aIndex, bIndex) == true)
		return;
	if(BotAlchemist.TradeOpen(aIndex, bIndex) == 1)
		return;
	if(BotTrader.TradeOpen(aIndex,bIndex) == 1)
		return;
	...
	if(lpObj->TradeLock == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index,1, 0, 0, 0, 0, 0, gMessage.GetMessage(541));
		return;
	}

	if(lpTarget->TradeLock == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index,1, 0, 0, 0, 0, 0, gMessage.GetMessage(542),lpTarget->Name);
		return;
	}
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg,int aIndex) // OK
{
	...
	RPSModeInface(lpObj->Index);
	RPSModeInface(lpTarget->Index);
	...
	if(lpObj->RPSMode >= 1 && lpTarget->RPSMode == 0)
	{
		this->ResetTrade(aIndex);
		this->GCTradeResultSend(aIndex,0);
		this->ResetTrade(bIndex);
		this->GCTradeResultSend(bIndex,0);
		gNotice.GCNoticeSend(bIndex,1,0,0,0,0,0,gMessage.GetMessage(615),lpObj->Name);
		return;
	}
	if(lpTarget->RPSMode >= 1 && lpObj->RPSMode == 0)
	{
		this->ResetTrade(aIndex);
		this->GCTradeResultSend(aIndex,0);
		this->ResetTrade(bIndex);
		this->GCTradeResultSend(bIndex,0);
		gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(615),lpTarget->Name);
		return;
	}
	if(lpObj->RPSMode >= 1 || lpTarget->RPSMode >= 1)
	{
		if(lpObj->RPSMode == lpTarget->RPSMode || lpTarget->RPSMode == lpObj->RPSMode)
		{
				this->ResetTrade(aIndex);
				this->GCTradeResultSend(aIndex,0);
				this->ResetTrade(bIndex);
				this->GCTradeResultSend(bIndex,0);
				gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(616));
				gNotice.GCNoticeSend(bIndex,1,0,0,0,0,0,gMessage.GetMessage(616));
		}

		if(lpObj->RPSMode == 3 && lpTarget->RPSMode == 1 || lpObj->RPSMode == 2 && lpTarget->RPSMode == 3 || lpObj->RPSMode == 1 && lpTarget->RPSMode == 2)
		{
			if(this->ExchangeTradeItem(lpTarget,lpTarget) == 0 || this->ExchangeTradeItem(lpObj,lpTarget) == 0)
			{
				this->ResetTrade(aIndex);
		...
		if(lpTarget->RPSMode == 1 && lpObj->RPSMode == 2 || lpTarget->RPSMode == 2 && lpObj->RPSMode == 3 || lpTarget->RPSMode == 3 && lpObj->RPSMode == 1)
		{
			if(this->ExchangeTradeItem(lpObj,lpObj) == 0 || this->ExchangeTradeItem(lpTarget,lpObj) == 0)
			{
				this->ResetTrade(aIndex);
				this->GCTradeResultSend(aIndex,2);
				this->ResetTrade(bIndex);
	...
	if(lpObj->RPSMode == 0 && lpTarget->RPSMode == 0)
	{
