...
void CKalima::KalimaGateAct(int aIndex) // OK
{
	...
	if(lpObj->KalimaGateEnterCount >= gServerInfo.m_KalimaMaxEnterUser)
	{
		this->DeleteKalimaGate(lpTarget->Index);
		return;
	}
	...
	if((GetTickCount()-lpObj->RegenTime) >= gServerInfo.m_KalimaMaxStand)
	{
		this->DeleteKalimaGate(lpTarget->Index);
		return;
	}
...
void CKalima::KalimaNextGateAct(int aIndex) // OK
{
	...
	if((GetTickCount()-lpObj->RegenTime) >= gServerInfo.m_KalimaMaxNextStand)
	{
		this->DeleteKalimaGate(lpTarget->Index);
		return;
	}
