...
void CDuel::CheckDuelArenaGladiator(DUEL_ARENA_INFO* lpInfo) // OK
{
	...
	for(int n=0;n < MAX_DUEL_ARENA_GLADIATOR;n++)
	{
		...
		{
			this->CheckDuelsBet2(&gObj[lpInfo->Gladiator[n]]);

			if(gObj[lpInfo->Gladiator[0]].Map == MAP_DUEL_ARENA)
			{
				this->CheckDuelsBet(&gObj[lpInfo->Gladiator[0]]);
			}

			if(gObj[lpInfo->Gladiator[1]].Map == MAP_DUEL_ARENA)
			{
				this->CheckDuelsBet(&gObj[lpInfo->Gladiator[1]]);
			}
...
void CDuel::CheckDuelScore(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->DuelScore_Bet >= gServerInfo.m_DuelBetMatchPoint)
	{
		gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,lpObj->DuelBetValue,0,0);
		gCashShop.GDCashShopSubPointSaveSend(lpTarget->Index,0,lpTarget->DuelBetValue,0,0);
	...
	if(lpObj->DuelScore >= gServerInfo.m_DuelMatchPoint)
	{
		this->ResetDuel(lpObj);
...
void CDuel::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpTarget->DuelBetValue >= 1)
	{
		lpTarget->DuelScore_Bet++;
	}
...
void CDuel::CGDuelStartRecv(PMSG_DUEL_START_RECV* lpMsg,int aIndex) // OK
{
	...
	lpTarget->DuelBetValue = 0;
	lpTarget->DuelScore_Bet = 0;
...
void CDuel::CGDuelBetsSend(int aIndex,int bIndex,DWORD Value) // OK
{
	if(gServerInfo.m_DuelSwitch == 0)
	{
		return;
	...
	if(lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(169));
		return;
	}

	if(lpObj->WCoinCaRecv < Value || lpTarget->WCoinCaRecv < Value )
	{
		return;
	}
	/*
...
*/
	...
	gNotice.GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,gMessage.GetMessage(633),lpObj->Name,Value);
	...
	lpTarget->DuelUser = -1;
	lpTarget->DuelTickCount = GetTickCount();
	lpTarget->DuelBetValue = Value;
	lpTarget->DuelScore_Bet = 0;

	this->GCDuelOkSend(bIndex,aIndex);
}

void CDuel::CheckDuelsBet(LPOBJ lpObj) // OK
{
	gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,lpObj->DuelBetValue,0,0);
}

void CDuel::CheckDuelsBet2(LPOBJ lpObj) // OK
{
