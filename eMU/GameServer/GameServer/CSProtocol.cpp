...
{
	//LogAdd(LOG_TEST,"%d %s",lpMsg->WindowGuid,lpMsg->Name);
	//LogAdd(LOG_TEST,"%s %d %d",lpMsg->Subject,lpMsg->Action,lpMsg->Dir);
	//LogAdd(LOG_TEST,"Size = %d [%s]",lpMsg->MemoSize,lpMsg->Memo);
	if ( !gObjIsConnectedGP(aIndex))
	{
		return;
	}
