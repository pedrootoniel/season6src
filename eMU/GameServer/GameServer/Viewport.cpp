...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpObj->VpPlayer[n].type != OBJECT_USER && gObj[lpObj->VpPlayer[n].index].IsBot==0)
		{
			continue;
		}
		...
		{
			info.DirAndPkLevel = (lpTarget->Dir*16) | (6 & 0x0F);
		}
		else if (lpObj->KillAll != 0 && lpTarget->KillAll != 0)
		{
			info.DirAndPkLevel = (lpTarget->Dir*16) | (6 & 0x0F);
		}
		else
...
void CViewport::GCViewportMonsterSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if((lpObj->VpPlayer[n].type != OBJECT_MONSTER && lpObj->VpPlayer[n].type != OBJECT_NPC) || gObj[lpObj->VpPlayer[n].index].IsBot != 0)
		{
			continue;
		}
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	if (lpObj->PvP != 0)
	{
		info.DirAndPkLevel = (lpObj->Dir*16) | (6 & 0x0F);
	}
	else if (lpObj->KillAll != 0)
	{
		info.DirAndPkLevel = (lpObj->Dir*16) | (6 & 0x0F);
	}
	else
