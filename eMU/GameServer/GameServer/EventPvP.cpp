...
void CEventPvP::MainProc() // OK
{
	...
	{ 
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(635));
			LogAdd(LOG_BLACK,"[EVENT PVP] Cancelled, no players");
			this->Clear();
		}
		...
		if (this->m_EventPvPEnter && this->m_Player1Index != -1 && this->m_Player2Index != -1) 
		{
			...
			this->m_EventPvPEnter = 0;
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(637), gObj[this->m_Player1Index].Name, gObj[this->m_Player2Index].Name);
			...
			LogAdd(LOG_BLACK,"[EVENT PVP] Started (%s x %s)", gObj[this->m_Player1Index].Name, gObj[this->m_Player2Index].Name);
		}
		...
		if(this->m_RemainTime <= 0)
		{
			    this->m_EventPvPActive = 0;
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(635));
				LogAdd(LOG_BLACK,"[EVENT PVP] Time is over");
		...
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(639),gObj[this->m_Player2Index].Name);
			...
			{	
				//GDSetCoinSend(this->m_Player2Index, gServerInfo.m_EventPvPAutoReward1, gServerInfo.m_EventPvPAutoReward2, gServerInfo.m_EventPvPAutoReward3);
				gCashShop.GDCashShopAddPointSaveSend(this->m_Player2Index,0,gServerInfo.m_EventPvPAutoReward1,gServerInfo.m_EventPvPAutoReward2,gServerInfo.m_EventPvPAutoReward3);
			}

			LogAdd(LOG_BLACK,"[EVENT PVP] (Winner: %s)",gObj[this->m_Player2Index].Name);
			this->Clear();
		}

		if (this->m_EventPvPActive && gObjIsConnectedGP(this->m_Player2Index) == 0) 
		{
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(639),gObj[this->m_Player1Index].Name);
			GCFireworksSend(&gObj[this->m_Player1Index],gObj[this->m_Player1Index].X,gObj[this->m_Player1Index].Y);

			if (gServerInfo.m_EventPvPAutoReward1 > 0 || gServerInfo.m_EventPvPAutoReward2 > 0 || gServerInfo.m_EventPvPAutoReward3 > 0)
			{	
				//GDSetCoinSend(this->m_Player1Index, gServerInfo.m_EventPvPAutoReward1, gServerInfo.m_EventPvPAutoReward2, gServerInfo.m_EventPvPAutoReward3);
				gCashShop.GDCashShopAddPointSaveSend(this->m_Player1Index,0,gServerInfo.m_EventPvPAutoReward1,gServerInfo.m_EventPvPAutoReward2,gServerInfo.m_EventPvPAutoReward3);
			}

			LogAdd(LOG_BLACK,"[EVENT PVP] (Winner: %s)",gObj[this->m_Player1Index].Name);
			this->Clear();
		}
...
bool CEventPvP::CommandEventPvP(LPOBJ lpObj,char* arg) // OK 
{
	...
	{
		this->Clear();
		LogAdd(LOG_BLACK,"[EVENT PVP] Canceled");

		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(636));
	...
	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(640));

	LogAdd(LOG_BLACK,"[EVENT PVP] Start by %s",lpObj->Name);
...
bool CEventPvP::EventPvPTradeJoin(int aIndex,int bIndex) // OK 
{
	...
	LogAdd(LOG_BLACK,"[EVENT PVP] Member add (%s)",lpObj->Name);
...
void CEventPvP::EventPvPDead(int aIndex,int bIndex) // OK 
{
	...
	{
		this->m_Score1++;
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(638),gObj[this->m_Player1Index].Name,this->m_Score1, this->m_Score2 ,gObj[this->m_Player2Index].Name);
