...
MONSTER_INFO* CMonsterManager::GetAttr(int aClass)
{
	for ( int n = 0; n<MAX_MONSTER_INFO; n++)
	{
		if ( this->m_MonsterInfo[n].Index == aClass )
		{
			return &this->m_MonsterInfo[n];
		}
	}
...
MONSTER_INFO* CMonsterManager::GetAttr(char * zsName)
{
	for ( int n = 0; n<MAX_MONSTER_INFO; n++)
	{
		if ( !strcmp(this->m_MonsterInfo[n].Name,zsName) )
		{
			return &this->m_MonsterInfo[n];
		}
	}
