...
void CCustomStore::CommandCustomStoreOffline(LPOBJ lpObj,char* arg) // OK
{
	...
	if(gMapManager.GetMapOffStoreEnable(lpObj->Map) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(569));
		return;
	}
...
void CCustomStore::OpenCustomStore(LPOBJ lpObj,int type) // OK
{
	...
	RPSMODE_INTERFACE pMsg2;
	pMsg2.header.set(0xFB,0x24,sizeof(pMsg2));
	DataSend(lpObj->Index,(BYTE*)&pMsg2,pMsg2.header.size);
