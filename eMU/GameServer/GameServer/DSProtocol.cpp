...
{	
	switch(head)
	{
		...
		case 0xD9:
			...
			{
			case 0x01:
				gBankEx.DGGetPoint((BANKEX_DG_GET_POINT*)lpMsg);
				break;
			case 0x02:
				gRanking.RecvDS((DGCharTop*)lpMsg);
				break;
			case 0x03:
				gDanhHieu.DGGetData((DANHHIEU_DG_GET_DATA*)lpMsg);
				break;
			case 0x04:
				gAccSecurity.SecurityData((ACC_SECURITY_DATA*)lpMsg);
				break;
			case 0x05:
				gFruitSystem.DGGetPoint((FRUIT_GET_POINT*)lpMsg);
				break;
			case 0x06:
				//GetTitanReward((SDHP_GETREWARD_INFOSAVE*)lpMsg);
				break;
			case 0x07:
				GiftCode_Reward((GIFTCODE_GETREWARD_INFOSAVE*)lpMsg);
				break;
			case 0x30:
				//UpdateQuestData((DS_SEND_QUESTS*)lpMsg);
				break;
			case 0x31:
				//UpdateQuestDataHunt((DS_SEND_QUESTS_HUNT*)lpMsg);
				break;			
			}
			break;
        case 0x78:
		...
		case 0x1B:
			...
			{
				case 0x00:
					gGoldenArcher.RecvGoldenArcher((PMSG_ANS_VIEW_EC_MN*)lpMsg);
					break;
				case 0x01:
					gGoldenArcher.DGResultRegEventChip((PMSG_ANS_REGISTER_EVENTCHIP*)lpMsg);
					break;
				case 0x02:
					gGoldenArcher.DGRecvChangeRena((PMSG_ANS_RESET_EVENTCHIP*)lpMsg);
					break;
			}
			break;
		case 0x1E:
...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg) // OK
{
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		*/
		...
		//else if(TempInventory[8] >= 147 && TempInventory[8] <= 257)
		//{
		//	info.CharSet[16] |= (TempInventory[8]-146) << 1;
		//}
		#pragma endregion
	...
	//DGNewPetInfoSend(lpMsg);

	GCCharacterCreationEnableSend(lpMsg->index,0,gObj[lpMsg->index].ClassCode);	
	gAccSecurity.GDReqData(lpMsg->index);
	gEventTimer.SendTime(lpMsg->index);
}

void GCCharacterInfoSend(SDHP_CHARACTER_LIST_RECV* lpMsg)
{
	BYTE send[4096];

	PMSG_CHARACTER_INFO_DATA pMsg;
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	gDanhHieu.GDReqData(lpObj->Index);
	gAccSecurity.GDReqData(lpObj->Index);
	//g_QuestSystem.ReqQuest(lpObj);
	//g_QuestSystem.ReqQuest_Hunt(lpObj);
	gBankEx.GDReqPoint(lpObj->Index);
	gFruitSystem.GDReqPoint(lpObj->Index);
	...
	if (lpObj->Reset < gServerInfo.m_ServerMinUserReset || lpObj->Reset > gServerInfo.m_ServerMaxUserReset)
	{
		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessage.GetMessage(571), gServerInfo.m_ServerMinUserReset, gServerInfo.m_ServerMaxUserReset);
		gObjectManager.CharacterGameCloseSet(lpObj->Index, 5);
		return;
	}

	if(gServerInfo.m_AnnouncePlayerJoinGame == 1)
	{
		char welcome[256];
		wsprintf(welcome,gServerInfo.m_AnnouncePlayerJoinGameText,lpObj->Name);
		gNotice.GCNoticeSendToAll(1,0,0,0,0,0,welcome);
	}

#if GAMESERVER_UPDATE == 603
	gPartySearch.SendPartySettings(lpObj);
	GCCharacterPageSend(lpObj);
	gChangeClass.SendData(lpObj->Index);
	gBuyAccountLevel.SendInfo(lpObj->Index);
	gEventTimer.SendTime(lpObj->Index);
	if( gServerInfo.NewBroad_ShowOnLogin )
	{
		gNewsBoard.OpenMain(lpObj);
	}
	if( gCustomQuest.Enable == 1)
	{
		if (lpObj->iQuestIndex > 0)
		{
			if (lpObj->iQuestState == 1 || lpObj->iQuestState == 2)
			{
				...
				{
					MONSTER_INFO* lpMA = gMonsterManager.GetInfo(gCustomQuest.m_Quest[lpObj->iQuestIndex].q_MobID);

					if (lpMA != NULL)
					{
						gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(673), lpObj->iQuestIndex);
						gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(658), gCustomQuest.m_Quest[lpObj->iQuestIndex].szHint);
						gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(655), gCustomQuest.m_Quest[lpObj->iQuestIndex].q_MobCnt, lpMA->Name);					
					}
				}
				else if (lpObj->iQuestVar < gCustomQuest.m_Quest[lpObj->iQuestIndex].q_MobCnt)
				{
					MONSTER_INFO* lpMA = gMonsterManager.GetInfo(gCustomQuest.m_Quest[lpObj->iQuestIndex].q_MobID);
					...
					{
						gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(673), lpObj->iQuestIndex);
						gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(667), lpMA->Name, lpObj->iQuestVar, gCustomQuest.m_Quest[lpObj->iQuestIndex].q_MobCnt);
					}
				}
				else
				{
					gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(670), lpObj->iQuestIndex);
					gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(671));
				}
			}
			else
			{
				if (gCustomQuest.GetQuest(lpObj->iQuestIndex))
				{
					gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(674));
				}
				else
				{
					gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(656));
				}
			}
		}
		else if (lpObj->iQuestIndex == 0)
		{
			gCustomQuest.SendMsg(lpObj->Index, 0, gMessage.GetMessage(669));
		}
	}
#endif
	...
	OFFEXP_DATA *info = gCustomAttack.GetOffExpInfo(lpObj);

	if (info != 0 && lpObj->Socket == INVALID_SOCKET)
	{
		//LogAdd(LOG_TEST,"INFO != 0");
		lpObj->IsOffExpReconnect = true;

		PMSG_OFFLINEMODE_DATA_RECV data;
		data.SkillID = info->SkillID;
		data.UseBuff = info->UseBuffs;
		...
		gOfflineMode.CGOfflineModeStartRecv(&data,lpObj->Index);

		/*
		if (gProtection.GetCustomState(CUSTOM_OFFLINE_MODE) != 0)
		{
			//LogAdd(LOG_TEST,"SetOffExp");
			gOfflineMode.CGOfflineModeStartRecv(&data,lpObj->Index);
		}
		else
		{
			gCustomAttack.CommandCustomAttack(lpObj,"");
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
	...
	if(lpMsg->Map == 0xFE || lpMsg->Map == 0xFF)
	{
		...
		if(lpMsg->ItemIndex == GET_ITEM(13,18) || lpMsg->ItemIndex == GET_ITEM(13,37) || lpMsg->ItemIndex == GET_ITEM(13,51) || lpMsg->ItemIndex == GET_ITEM(14,7) || lpMsg->ItemIndex == GET_ITEM(14,19) || lpMsg->ItemIndex == GET_ITEM(14,109) ||  lpMsg->ItemIndex >= GET_ITEM(13,150) && lpMsg->ItemIndex <= GET_ITEM(13,450))
		{
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	switch(lpMsg->type)
	{
		...
		case 3:
			...
			break;
		case 4:
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,"[%s] %s",lpMsg->name,lpMsg->message);
			break;
	}
}
...
void GDCharacterListMultiSend(PMSG_REQ_MULTI_CHAR_RECV* lpMsg, int aIndex) // OK
{
	if(gObj[aIndex].Connected != OBJECT_LOGGED)
	{
		return;
	}
	...
	SDHP_CHARACTER_LIST_MULTI_SEND pMsg;

	pMsg.header.set(0xE3,sizeof(pMsg));

	pMsg.index = aIndex;

	pMsg.Page = lpMsg->Page;
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	pMsg.IsMarried = lpObj->IsMarried;
	memcpy(pMsg.MarryName,lpObj->MarryName,sizeof(pMsg.MarryName));

	pMsg.iQuestIndex = lpObj->iQuestIndex;
	pMsg.iQuestState = lpObj->iQuestState;
	pMsg.iQuestVar = lpObj->iQuestVar;
	pMsg.iQuestStat = lpObj->iQuestStat;
	...
	gFruitSystem.GDSavePoint(aIndex);
}
...
void GDSaveSecurityPass(int aIndex)
{
	LPOBJ lpUser = &gObj[aIndex];
	ACCSECURITY_PASS_SAVE pRequest;
	pRequest.header.set(0xD9, 0x14, sizeof(pRequest));
	memcpy(pRequest.account, lpUser->Account, 11);
	pRequest.index	= aIndex;
	pRequest.SecurityPass = lpUser->SecurityPass;
	gDataServerConnection.DataSend((BYTE*)&pRequest,pRequest.header.size);
}


void DelGiftCode(int aIndex, int ID_Num)
{
	SDHP_GIFTCODE_INFODEL pMsg = {0};
	pMsg.h.set(0xD9, 0x22, sizeof(pMsg));
	pMsg.ID_Num = ID_Num;
	gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.h.size);
}

void GiftCode_Sen(int aIndex, int ID)
{
	GiftCode_GetInfo pMsg = {0};
	pMsg.h.set(0xD9, 0x23, sizeof(pMsg));
...
void GiftCodeCount(int ID_Num)
{
	GIFTCODE_COUNT pMsg = {0};
	pMsg.h.set(0xD9, 0x24, sizeof(pMsg));
	pMsg.ID = ID_Num;
	gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.h.size);
}

void GiftCodeLog(int aIndex, int ID_Num)
{
	GIFTCODE_LOG pMsg = {0};
	pMsg.h.set(0xD9, 0x25, sizeof(pMsg));
	memcpy(pMsg.Name, gObj[aIndex].Name, 11);
	pMsg.ID = ID_Num;
	gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.h.size);
}

void GiftCode_Reward(GIFTCODE_GETREWARD_INFOSAVE * lpMsg)
{	
	int aIndex = 0;
	aIndex = lpMsg->aIndex;
	...
	LPOBJ lpObj = &gObj[aIndex];
		...
		if(lpMsg->ID_Num >= 0)
		{
			if(lpMsg->rew.IDCount >= 1)
			{/*
				if(lpMsg->rew.Days == 0)
				{
					GDCreateItemSend(lpObj->Index,0xEB,0,0,lpMsg->rew.ItemCode,lpMsg->rew.Level,0,lpMsg->rew.Skill,lpMsg->rew.Luck,lpMsg->rew.Opt,-1,lpMsg->rew.Exc,0,0,0,0,0xFF,0);
					GiftCodeCount(lpMsg->ID_Num);
					GiftCodeLog(lpObj->Index,lpMsg->ID_Num);
				...
				else
				{
					...
					GDCreateItemSend(lpObj->Index,0xEB,0,0,lpMsg->rew.ItemCode,lpMsg->rew.Level,0,lpMsg->rew.Skill,lpMsg->rew.Luck,lpMsg->rew.Opt,-1,lpMsg->rew.Exc,0,0,0,0,0xFF,iTime);
					GiftCodeCount(lpMsg->ID_Num);
					GiftCodeLog(lpObj->Index,lpMsg->ID_Num);
					GCFireworksSend(lpObj,lpObj->X,lpObj->Y);
				}*/
				gItemBagManager.DropItemByIndex(lpMsg->rew.ItemBagIndex,lpObj,lpObj->Map,lpObj->X,lpObj->Y);
				if(lpMsg->rew.Zen > 1 || lpMsg->rew.wCoinC > 1 || lpMsg->rew.wCoinP > 1 || lpMsg->rew.GoblinPoint > 1)
				{
					if(gObjCheckMaxMoney(lpObj->Index,lpMsg->rew.Zen) == 0)
					{
						gObj[lpObj->Index].Money = MAX_MONEY;
					}
					else
					{
						gObj[lpObj->Index].Money += lpMsg->rew.Zen;
					}
					GCMoneySend(aIndex, lpObj->Money);
					gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,lpMsg->rew.wCoinC,lpMsg->rew.wCoinP,lpMsg->rew.GoblinPoint);
					gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage.GetMessage(572),lpMsg->rew.Zen,lpMsg->rew.wCoinC,lpMsg->rew.wCoinP,lpMsg->rew.GoblinPoint);
				}
				if(lpMsg->rew.BuffID > 1)
				{
					time_t t = time(NULL);
					localtime(&t);
					DWORD BuffTime = (DWORD)t + lpMsg->rew.BuffTime * 86400;
					gEffectManager.AddEffect(lpObj,1,lpMsg->rew.BuffID,(int)BuffTime,0,0,0,0);
				}
				LogAdd(LOG_RED,"[GiftCode](%s)(%s) Active Gift ID:%d",gObj[aIndex].Account,gObj[aIndex].Name,lpMsg->ID_Num);
			}
			else
			{
				DelGiftCode(aIndex,lpMsg->ID_Num);
			}
		}
		else
		{
			gNotice.GCNoticeSend(aIndex,0,0,0,0,0,0,gMessage.GetMessage(573));
		}
}
