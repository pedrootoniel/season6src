...
struct PSBMSG_HEAD2
{
	void set(LPBYTE lpBuf,BYTE head,BYTE subh,BYTE size) // OK
	{
		this->type = 0xC1;
		this->size = size;
		this->head = head;
	...
	void setE(LPBYTE lpBuf,BYTE head,BYTE subh,BYTE size) // OK
	{
		this->type = 0xC3;
		this->size = size;
		this->head = head;
...
struct PMSG_BALANCE_UPDATE_SEND
{
	PSBMSG_HEAD header;
	DWORD	wCoinC;
	DWORD	wCoinP;
...
struct PMSG_ACCSECURITY_OPEN
{
	PSBMSG_HEAD header;
};

struct PMSG_CHARACTERPAGE_SEND
{
	PSBMSG_HEAD	header;
	int Page;
};

struct PMSG_REQ_MULTI_CHAR_RECV
{
	PSBMSG_HEAD header;
	int Page;
};

struct PMSG_RENAME_RECV
{
	PSBMSG_HEAD header;
	char Name[11];
};
