...
void CMove::Move(LPOBJ lpObj,int index) // OK
{
	...
	if ( g_Survivor.IsInSurvivorArea(&gObj[index]) )
	{
		g_Survivor.ClearPlayerData(&gObj[index]);
	}
...
void CMove::CGTeleportRecv(PMSG_TELEPORT_RECV* lpMsg,int aIndex) // OK
{
	...
	if( g_Survivor.IsInSurvivorArea(&gObj[aIndex]) )
	{
		if(gServerInfo.SurvivorDisableTeleport == 1)
		{
			return;
		}
	}
