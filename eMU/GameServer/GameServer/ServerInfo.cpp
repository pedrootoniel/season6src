...
char g_szMapName[90][90] =
{
	"Lorencia","Dungeon","Devias","Noria","LostTower","Exile","Arena","Atlans","Tarkan",
	"Devil Square 1","Icarus","BloodCastle 1","BloodCastle 2","BloodCastle 3","BloodCastle 4",
	"BloodCastle 5","BloodCastle 6","BloodCastle 7","ChaosCastle 1","ChaosCastle 2","ChaosCastle 3",
	"ChaosCastle 4","ChaosCastle 5","ChaosCastle 6","Kalima 1","Kalima 2","Kalima 3","Kalima 4","Kalima 5",
	"Kalima 6","Valley Of Loren","Land Of Trial","Devil Square 2","Aida","Crywolf","None","Kalima 7","Kanturu 1",
	"Kanturu 2","Kanturu 3","GM Map","Balgass Baracks","Balgass Refuge","None","None","Illusion Temple 1","Illusion Temple 2",
	"Illusion Temple 3","Illusion Temple 4","Illusion Temple 5","Illusion Temple 6","Elbeland","BloodCastle 8","Chaos Castle 7",
	"None","None","Swamp of Calmness","Raklion","Raklion Boss","None","None","None","Santa Town","Vulcanus","Duel Arena""Doppelganger",
	"Doppelganger","Doppelganger","Doppelganger","Empire Fortress","Empire Fortress","Empire Fortress","Empire Fortress",
	"None","None","None","None","None","None","None","Loren Market","Karutan 1","Karutan 2","Arkania","Acheron","Deventer","Uruk Mountain","Ferea",
};
...
void CServerInfo::ReadCommandInfo() // OK
{
	...
	gCommandManager.Load("..\\Data\\Custom\\CommandManager.xml");
...
void CServerInfo::ReadCustomInfo() // OK
{
	...
	gDanhHieu.Load(gPath.GetFullPath("Custom\\CustomTitle.txt"));
	...
	gTvTEvent.Load(gPath.GetFullPath("..\\Data\\Custom\\TvTEvent.txt"));

	gChangeClass.Load(gPath.GetFullPath("..\\Data\\Custom\\ChangeClass.txt"));

	gBuyAccountLevel.Load(gPath.GetFullPath("..\\Data\\Custom\\BuyVip.txt"));

	gTvTEvent.ReadTvTEventInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.dat");

	gOfflineMode.LoadSkillsInfo("..\\Data\\Custom\\OfflineMode\\OfflineModeBuffs.xml");
	gOfflineMode.Load("OfflineMode","..\\Data\\Custom\\OfflineMode\\OfflineMode.ini");

	//SVShop.Init("..\\Data\\Custom\\SmithItem\\SmithItem.ini","..\\Data\\Custom\\SmithItem\\SmithItem.txt");

	gQuaTanThu.Load(gPath.GetFullPath("Custom\\TheGift.txt"));

	gCustomMove.Load(gPath.GetFullPath("Custom\\CustomMove.txt"));

	//g_QuestSystem.Load();

	this->ReadPKEventInfo();

	this->ReadInstaGameInfo();

	this->ReadClientInterfaceInfo();

	gPartySearch.Load("Config","..\\Data\\Custom\\PartySearch.txt");

	gCustomRankUser.Load(gPath.GetFullPath("..\\Data\\Custom\\ResetReward.txt"));

	gSummonScroll.Load(gPath.GetFullPath("Custom\\SummonScroll.txt"));

	g_ShopPointEx.Load();

	gBotBuffer.Load(gPath.GetFullPath("Custom\\BotBuffer.txt"));
	gBotBuffer.MakeBot();

	BotAlchemist.Load(gPath.GetFullPath("Custom\\BotAlchemist.txt"));
	BotAlchemist.MakeBot();

	//BotStore.Read(gPath.GetFullPath("Custom\\BotStore.txt"));
	//BotStore.MakeBot();

	BotTrader.Load(gPath.GetFullPath("Custom\\BotTrader.txt"));
	BotTrader.MakeBot();
	...
	Marry.LoadFile(gPath.GetFullPath("Custom\\Marry.xml"));

	gChaosBoxManager.Read(gPath.GetFullPath("Custom\\ChaosMachineManager.xml"));

	gLuckySpin.Load();

	gPVPZone.Load(gPath.GetFullPath("Custom\\PVPZone.txt"));

	//g_UserDieItemDrop.Load();

	gEventTimer.Load(gPath.GetFullPath("Custom\\EventTimer.txt"));
...
void CServerInfo::ReadSkillInfo() // OK
{
	...
	//g_ComboSkillPath.Load(gPath.GetFullPath("Skill\\ComboSkill.txt"));
...
void CServerInfo::ReadUtilInfo() // OK
{
	...
	gCustomDeathMessage.Load(gPath.GetFullPath("Util\\CustomDeathMessage.txt"));
...
void CServerInfo::ReloadMonsterInfo() // OK
{
	...
	for ( int n=MAX_OBJECT_MONSTER;n<OBJECT_START_USER;n++)
	{
		if ( gObj[n].Type == OBJECT_MONSTER || gObj[n].Type == OBJECT_NPC )
		{
			if(gObj[n].IsBot >= 1 && gObj[n].BotNumOwner <= 0)
			{
				if ( gObj[n].CurrentAI )
				{
					gObj[n].Live = FALSE;
...
void CServerInfo::ReadStartupInfo(char* section,char* path) // OK
{
	...
	this->m_ServerMinUserReset = GetPrivateProfileInt(section,"ServerMinReset",0,path);

	this->m_ServerMaxUserReset = GetPrivateProfileInt(section,"ServerMaxReset",0,path);
	...
	this->m_WriteInstaGamesLog = GetPrivateProfileInt(section,"WriteInstaGamesLog",0,path);

	this->m_WriteSmithItemLog = GetPrivateProfileInt(section,"WriteSmithItemLog",0,path);

	this->m_WriteQuestLog = GetPrivateProfileInt(section,"WriteCustomQuestLog",0,path);
...
void CServerInfo::ReadChaosMixInfo(char* section,char* path) // OK
{
	...
	this->m_SumWingMix = GetPrivateProfileInt(section,"SumWingMix",0,path);

	this->m_RFWingMix = GetPrivateProfileInt(section,"RFWingMix",0,path);
	...
	this->m_ArchangelHammerMix[0] = GetPrivateProfileInt(section,"ArcHammerMixRate_AL0",0,path);

	this->m_ArchangelHammerMix[1] = GetPrivateProfileInt(section,"ArcHammerMixRate_AL1",0,path);

	this->m_ArchangelHammerMix[2] = GetPrivateProfileInt(section,"ArcHammerMixRate_AL2",0,path);

	this->m_ArchangelHammerMix[3] = GetPrivateProfileInt(section,"ArcHammerMixRate_AL3",0,path);
	
	this->m_ArchangelWeaponMix[0] = GetPrivateProfileInt(section,"ArcWeaponMixRate_AL0",0,path);

	this->m_ArchangelWeaponMix[1] = GetPrivateProfileInt(section,"ArcWeaponMixRate_AL1",0,path);

	this->m_ArchangelWeaponMix[2] = GetPrivateProfileInt(section,"ArcWeaponMixRate_AL2",0,path);

	this->m_ArchangelWeaponMix[3] = GetPrivateProfileInt(section,"ArcWeaponMixRate_AL3",0,path);

	this->m_FeatherOfGarudaMixRate[0] = GetPrivateProfileInt(section,"FeatherOfGarudaMixRate_AL0",0,path);

	this->m_FeatherOfGarudaMixRate[1] = GetPrivateProfileInt(section,"FeatherOfGarudaMixRate_AL1",0,path);

	this->m_FeatherOfGarudaMixRate[2] = GetPrivateProfileInt(section,"FeatherOfGarudaMixRate_AL2",0,path);

	this->m_FeatherOfGarudaMixRate[3] = GetPrivateProfileInt(section,"FeatherOfGarudaMixRate_AL3",0,path);
...
void CServerInfo::ReadCommandInfo(char* section,char* path) // OK
{
	...
	this->m_PostItem	=	GetPrivateProfileInt(section,"PostItemSwitch",0,path);
	...
	this->m_CommandAddPointMoney[0] = GetPrivateProfileInt(section,"CommandAddPointAutoMoney_AL0",0,path);

	this->m_CommandAddPointMoney[1] = GetPrivateProfileInt(section,"CommandAddPointAutoMoney_AL1",0,path);

	this->m_CommandAddPointMoney[2] = GetPrivateProfileInt(section,"CommandAddPointAutoMoney_AL2",0,path);

	this->m_CommandAddPointMoney[3] = GetPrivateProfileInt(section,"CommandAddPointAutoMoney_AL3",0,path);
	...
	this->m_CustomResetRewardSwitch = GetPrivateProfileInt(section,"ResetRewardSwitch",0,path);
	...
	this->m_CommandClearINVMoney[0] = GetPrivateProfileInt(section,"CommandClearINVMoney_AL0",0,path);

	this->m_CommandClearINVMoney[1] = GetPrivateProfileInt(section,"CommandClearINVMoney_AL1",0,path);

	this->m_CommandClearINVMoney[2] = GetPrivateProfileInt(section,"CommandClearINVMoney_AL2",0,path);

	this->m_CommandClearINVMoney[3] = GetPrivateProfileInt(section,"CommandClearINVMoney_AL3",0,path);

	this->m_CommandResetPoint2	=	GetPrivateProfileInt(section,"CommandResetPointSwitch",0,path);
	this->m_CommandResetSkill2	=	GetPrivateProfileInt(section,"CommandClearSkillSwitch",0,path);
	this->m_CommandResetMasterSkill2	=	GetPrivateProfileInt(section,"CommandResetMasterSkillSwitch",0,path);

	this->m_CommandFreePointSwitch[0]	 =	GetPrivateProfileInt(section,"CommandResetPoint_AL0",0,path);
	this->m_CommandFreePointSwitch[1]	 =	GetPrivateProfileInt(section,"CommandResetPoint_AL1",0,path);
	this->m_CommandFreePointSwitch[2]	 =	GetPrivateProfileInt(section,"CommandResetPoint_AL2",0,path);
	this->m_CommandFreePointSwitch[3]	 =	GetPrivateProfileInt(section,"CommandResetPoint_AL3",0,path);
	this->m_CommandFreePointWcoinP[0]	 =	GetPrivateProfileInt(section,"CommandResetPointWC_AL0",0,path);
	this->m_CommandFreePointWcoinP[1]	 =	GetPrivateProfileInt(section,"CommandResetPointWC_AL1",0,path);
	this->m_CommandFreePointWcoinP[2]	 =	GetPrivateProfileInt(section,"CommandResetPointWC_AL2",0,path);
	this->m_CommandFreePointWcoinP[3]	 =	GetPrivateProfileInt(section,"CommandResetPointWC_AL3",0,path);
	this->m_CommandFreePointZen[0]	 =	GetPrivateProfileInt(section,"CommandResetPointZen_AL0",0,path);
	this->m_CommandFreePointZen[1]	 =	GetPrivateProfileInt(section,"CommandResetPointZen_AL1",0,path);
	this->m_CommandFreePointZen[2]	 =	GetPrivateProfileInt(section,"CommandResetPointZen_AL2",0,path);
	this->m_CommandFreePointZen[3]	 =	GetPrivateProfileInt(section,"CommandResetPointZen_AL3",0,path);

	this->m_CommandFreeSkillSwitch[0]	 =	GetPrivateProfileInt(section,"CommandClearSkill_AL0",0,path);
	this->m_CommandFreeSkillWcoinP[0]	 =	GetPrivateProfileInt(section,"CommandClearSkillMoney_AL0",0,path);
	this->m_CommandFreeSkillSwitch[1]	 =	GetPrivateProfileInt(section,"CommandClearSkill_AL1",0,path);
	this->m_CommandFreeSkillWcoinP[1]	 =	GetPrivateProfileInt(section,"CommandClearSkillMoney_AL1",0,path);
	this->m_CommandFreeSkillSwitch[2]	 =	GetPrivateProfileInt(section,"CommandClearSkill_AL2",0,path);
	this->m_CommandFreeSkillWcoinP[2]	 =	GetPrivateProfileInt(section,"CommandClearSkillMoney_AL2",0,path);
	this->m_CommandFreeSkillSwitch[3]	 =	GetPrivateProfileInt(section,"CommandClearSkill_AL3",0,path);
	this->m_CommandFreeSkillWcoinP[3]	 =	GetPrivateProfileInt(section,"CommandClearSkillMoney_AL3",0,path);

	this->m_CommandFreeMasterSwitch[0]	 =	GetPrivateProfileInt(section,"CommandResetMasterSkill_AL0",0,path);
	this->m_CommandFreeMasterWcoinP[0]	 =	GetPrivateProfileInt(section,"CommandResetMasterSkillMoney_AL0",0,path);
	this->m_CommandFreeMasterSwitch[1]	 =	GetPrivateProfileInt(section,"CommandResetMasterSkill_AL1",0,path);
	this->m_CommandFreeMasterWcoinP[1]	 =	GetPrivateProfileInt(section,"CommandResetMasterMoneySkill_AL1",0,path);
	this->m_CommandFreeMasterSwitch[2]	 =	GetPrivateProfileInt(section,"CommandResetMasterSkill_AL2",0,path);
	this->m_CommandFreeMasterWcoinP[2]	 =	GetPrivateProfileInt(section,"CommandResetMasterMoneySkill_AL2",0,path);
	this->m_CommandFreeMasterSwitch[3]	 =	GetPrivateProfileInt(section,"CommandResetMasterSkill_AL3",0,path);
	this->m_CommandFreeMasterWcoinP[3]	 =	GetPrivateProfileInt(section,"CommandResetMasterMoneySkill_AL3",0,path);
	...
	//THE GIFT
	this->m_TheGift = GetPrivateProfileInt(section,"CommandTheGiftSwitch",0,path);
	this->m_TheGiftEnable[0] = GetPrivateProfileInt(section,"CommandTheGift_AL0",0,path);
	this->m_TheGiftEnable[1] = GetPrivateProfileInt(section,"CommandTheGift_AL1",0,path);
	this->m_TheGiftEnable[2] = GetPrivateProfileInt(section,"CommandTheGift_AL2",0,path);
	this->m_TheGiftEnable[3] = GetPrivateProfileInt(section,"CommandTheGift_AL3",0,path);
	//
	// RENAME
	...
	//BLOC ACC
	this->m_CommandBlocAccSwitch			= GetPrivateProfileInt(section,"CommandBlockAccSwitch",0,path);
	this->m_CommandBlocAccGameMasterLevel	= GetPrivateProfileInt(section,"CommandBlockAccGameMasterLevel",0,path);
	//BLOC CHAR
	this->m_CommandBlocCharSwitch			= GetPrivateProfileInt(section,"CommandBlockCharSwitch",0,path);
	this->m_CommandBlocCharGameMasterLevel	= GetPrivateProfileInt(section,"CommandBlockCharGameMasterLevel",0,path);
}
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_TitleBarSwitch = GetPrivateProfileInt(section,"TitleBarSwitch",0,path);
	this->m_PlayerHPBarSwitch = GetPrivateProfileInt(section,"PlayerHPBarSwitch",0,path);
	this->m_BypassShield = GetPrivateProfileInt(section,"AttackByPassShield",0,path);
	#endif
	...
	this->m_DuelMatchPoint = GetPrivateProfileInt(section,"DuelMatch",0,path);

	this->m_DuelBetMatchPoint = GetPrivateProfileInt(section,"DuelBetMatch",0,path);
	...
	this->m_MaxPartyLevelDiff = GetPrivateProfileInt(section,"PartyMaxLevelDiff",0,path);

	this->m_AcceptPartyKill = GetPrivateProfileInt(section,"AcceptPartyKill",0,path);

	//this->GetQuestKillOutsideParty = GetPrivateProfileInt(section,"GetQuestKillOutsidePartyZone",0,path);
	...
	GetPrivateProfileString("GameServerInfo","GuardMessage","Welcome to Ferrarezi eMUlador",this->GuardMessage,sizeof(this->GuardMessage),path);

	this->VaultOpenDelay = GetPrivateProfileInt(section,"VaultOpenDelay",0,path);
	...
	this->LuckyItemTrade = GetPrivateProfileInt(section,"AllowLuckyItemTrade",0,path);

	this->LuckyItemVault = GetPrivateProfileInt(section,"AllowLuckyItemVault",0,path);

	this->LuckyItemAcceptJoh = GetPrivateProfileInt(section,"AllowLuckyItemAcceptHarmony",0,path);

	this->FruitOriginalWZ = GetPrivateProfileInt(section,"FruitOriginalWZ",0,path);

	this->m_AntihackCRC = GetPrivateProfileInt(section,"MHPClientDllCRC",0,path);

	this->m_DllCRC = GetPrivateProfileInt(section,"MainDllCRC",0,path);

	this->m_MainCRC = GetPrivateProfileInt(section,"MainExeCRC",0,path);

	this->m_PlayerCRC = GetPrivateProfileInt(section,"PlayerBMDCRC",0,path);

	this->m_DisconnectOnConenct = GetPrivateProfileInt(section,"DisconnectOnConnect",0,path);
}
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_AnnouncePlayerJoinGame = GetPrivateProfileInt(section,"AnnouncePlayerJoinGame",0,path);

	GetPrivateProfileString(section,"AnnouncePlayerJoinGameText","",this->m_AnnouncePlayerJoinGameText,sizeof(this->m_AnnouncePlayerJoinGameText),path);

	this->m_AnnouncePlayerKillPlayer = GetPrivateProfileInt(section,"AnnouncePlayerKillPlayer",0,path);

	GetPrivateProfileString(section,"AnnouncePlayerKillPlayerText","",this->m_AnnouncePlayerKillPlayerText,sizeof(this->m_AnnouncePlayerKillPlayerText),path);
	...
	//this->CustomCombo = GetPrivateProfileInt(section,"CustomCombo",0,path);
}
...
void CServerInfo::ReadEventInfo(char* section,char* path) // OK
{
	...
	this->m_CastleSiegeSummonRoom = GetPrivateProfileInt(section,"SummonCastleSiegeRoom",0,path);
	...
	this->m_KalimaMaxEnterUser = GetPrivateProfileInt(section,"MaxKalimaEnterUser",0,path);

	this->m_KalimaMaxStand = GetPrivateProfileInt(section,"MaxKalimaStand",0,path) * 1000;

	this->m_KalimaMaxNextStand = GetPrivateProfileInt(section,"MaxKalimaNextStand",0,path) * 1000;
	...
	#endif
	this->m_CustomQuizSwitch =  GetPrivateProfileInt(section,"QuizEvent",0,path);
	//
	...
	this->m_EventPvPAutoReward1				= GetPrivateProfileInt(section,"EventPvPRewardWC",0,path);
	this->m_EventPvPAutoReward2				= GetPrivateProfileInt(section,"EventPvPRewardWP",0,path);
	this->m_EventPvPAutoReward3				= GetPrivateProfileInt(section,"EventPvPRewardGP",0,path);
	//
	...
	this->m_EventKillAllAutoReward1[0]		= GetPrivateProfileInt(section,"EventKillAllRewardWCRank1",0,path);
	this->m_EventKillAllAutoReward1[1]		= GetPrivateProfileInt(section,"EventKillAllRewardWCRank2",0,path);
	this->m_EventKillAllAutoReward1[2]		= GetPrivateProfileInt(section,"EventKillAllRewardWCRank3",0,path);
	this->m_EventKillAllAutoReward2[0]		= GetPrivateProfileInt(section,"EventKillAllRewardWPRank1",0,path);
	this->m_EventKillAllAutoReward2[1]		= GetPrivateProfileInt(section,"EventKillAllRewardWPRank2",0,path);
	this->m_EventKillAllAutoReward2[2]		= GetPrivateProfileInt(section,"EventKillAllRewardWPRank3",0,path);
	this->m_EventKillAllAutoReward3[0]		= GetPrivateProfileInt(section,"EventKillAllRewardGPRank1",0,path);
	this->m_EventKillAllAutoReward3[1]		= GetPrivateProfileInt(section,"EventKillAllRewardGPRank2",0,path);
	this->m_EventKillAllAutoReward3[2]		= GetPrivateProfileInt(section,"EventKillAllRewardGPRank3",0,path);
	//Pk enter in events
	this->m_PkCanEnterBC	= GetPrivateProfileInt(section,"PKCanEnterBloodCastle",0,path);
	this->m_PkCanEnterCC	= GetPrivateProfileInt(section,"PKCanEnterChaosCastle",0,path);
	this->m_PkCanEnterDS	= GetPrivateProfileInt(section,"PKCanEnterDevilSquare",0,path);
	this->m_GoldenArcherNeedRena = GetPrivateProfileInt(section,"GoldenArcherNeedRenaCount",0,path);
}
...
void CServerInfo::ReadSkillInfo(char* section,char* path) // OK
{
	...
	this->CyclonePushBack = GetPrivateProfileInt(section,"CyclonePushBack",0,path);
	...
	#endif
	this->UsingComboSkill  = GetPrivateProfileInt(section,"UsingCombo",0,path);
	//this->m_ComboSkillPath = GetPrivateProfileInt(section,"ComboSkillType",0,path);
...
{
	GetPrivateProfileString("MESSAGES","Msg01","[Survivor][%s] Event starts in %d minutes",this->SurvivorMsg[0],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg02","[Survivor][%s] Enter /survivor to join the Event",this->SurvivorMsg[1],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg03","All Classes",this->SurvivorMsg[2],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg04","Wizard",this->SurvivorMsg[3],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg05","Knight",SurvivorMsg[4],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg06","Elf",SurvivorMsg[5],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg07","Gladiator",this->SurvivorMsg[6],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg08","Dark Lord",this->SurvivorMsg[7],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg09","Rage Fighter",this->SurvivorMsg[8],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg10","The current Survivor event is not for your class",this->SurvivorMsg[9],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg11","No Survivor event is currently opened",this->SurvivorMsg[10],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg12","You are signed to Survivor! Good luck!",this->SurvivorMsg[11],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg13","You are already signed for next Survivor!",this->SurvivorMsg[12],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg14","[Survivor] Battle starts in %d minutes. Prepare yourself!",this->SurvivorMsg[13],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg15","[Survivor] Battle has started. Save yourself!",this->SurvivorMsg[14],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg16","[%s] killed [%s]",this->SurvivorMsg[15],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg17","You were killed by %s",this->SurvivorMsg[16],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg18","[Survivor] [%s] is the Survivor, and Wins!",this->SurvivorMsg[17],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg19","[Survivor] [%s] Wins by kill count",this->SurvivorMsg[18],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg20","[Survivor] Event ended without winner at this time",this->SurvivorMsg[19],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg21","[Survivor] Event canceled due to lack of players",this->SurvivorMsg[20],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg22","You cant clear PK during the Survivor event",this->SurvivorMsg[21],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");
	GetPrivateProfileString("MESSAGES","Msg23","The current Survivor event is not for your class",this->SurvivorMsg[22],sizeof(this->SurvivorMsg[0]),"..\\Data\\Custom\\PKEvent.txt");	
	this->SurvivorMap	= GetPrivateProfileInt("OPTIONS","SurvivorMap",0,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorX1	= GetPrivateProfileInt("OPTIONS","SurvivorX1",136,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorY1	= GetPrivateProfileInt("OPTIONS","SurvivorY1",123,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorX2	= GetPrivateProfileInt("OPTIONS","SurvivorX2",145,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorY2	= GetPrivateProfileInt("OPTIONS","SurvivorY2",131,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorTimeLimit			= GetPrivateProfileInt("OPTIONS","TimeLimit",5,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorWinnerByKillCount	= GetPrivateProfileInt("OPTIONS","WinnerByKillCount",1,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorMinimumPlayers	= GetPrivateProfileInt("OPTIONS","MinimumPlayers",1,"..\\Data\\Custom\\PKEvent.txt");
	this->SurvivorDisableTeleport	= GetPrivateProfileInt("OPTIONS","DisableTeleport",1,"..\\Data\\Custom\\PKEvent.txt");
	g_Survivor.wCoinC		= GetPrivateProfileInt("PRIZE","wCoinC",1,"..\\Data\\Custom\\PKEvent.txt");
	g_Survivor.wCoinP		= GetPrivateProfileInt("PRIZE","wCoinP",1,"..\\Data\\Custom\\PKEvent.txt");
	g_Survivor.GoblinCoin		= GetPrivateProfileInt("PRIZE","GoblinCoin",1,"..\\Data\\Custom\\PKEvent.txt");
}
void CServerInfo::ReadInstaGameInfo() // OK
{
	// Roll the Dice
	gRolltheDice.enable = GetPrivateProfileInt("RolltheDice","Enable",0,"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("RolltheDice","Msg01","Roll-Dice gambling has disable!",gRolltheDice.msg[0],sizeof(gRolltheDice.msg[0]),"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("RolltheDice","Msg02","You need %d wCoinC to roll!",gRolltheDice.msg[1],sizeof(gRolltheDice.msg[1]),"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("RolltheDice","Msg03","[Roll-Dice] Player: %s WINNER %d wCoinC",gRolltheDice.msg[2],sizeof(gRolltheDice.msg[2]),"..\\Data\\Custom\\InstaGames.txt");
	// Lucky Number
	gLuckyNumber.Enable = GetPrivateProfileInt("LuckyNumber","Enable",0,"..\\Data\\Custom\\InstaGames.txt");
	gLuckyNumber.WinPrice = GetPrivateProfileInt("LuckyNumber","WinPrice",0,"..\\Data\\Custom\\InstaGames.txt");
	gLuckyNumber.TicketPrice = GetPrivateProfileInt("LuckyNumber","TicketPrice",0,"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("LuckyNumber","Msg01","Lucky-Number gambling has disable!",gLuckyNumber.msg[0],sizeof(gLuckyNumber.msg[0]),"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("LuckyNumber","Msg02","You need %d wCoinC to generator number!",gLuckyNumber.msg[1],sizeof(gLuckyNumber.msg[1]),"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("LuckyNumber","Msg03","[LOTTO] Generator new lucky number: %d",gLuckyNumber.msg[2],sizeof(gLuckyNumber.msg[2]),"..\\Data\\Custom\\InstaGames.txt");
	// Heads Tails
	gHeadsTails.enable = GetPrivateProfileInt("HeadsTails","Enable",0,"..\\Data\\Custom\\InstaGames.txt");
	gHeadsTails.winrate = GetPrivateProfileInt("HeadsTails","WinRate",0,"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("HeadsTails","Msg01","Heads-Tails gambling has disable!",gHeadsTails.msg[0],sizeof(gHeadsTails.msg[0]),"..\\Data\\Custom\\InstaGames.txt");
	GetPrivateProfileString("HeadsTails","Msg02","You need %d wCoinC to flip coin!",gHeadsTails.msg[1],sizeof(gHeadsTails.msg[1]),"..\\Data\\Custom\\InstaGames.txt");
}

void CServerInfo::ReadClientInterfaceInfo() // OK
{
	//Interface Button
	this->m_SettingsButton = GetPrivateProfileInt("Interface","SettingsButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_RolltheDiceButton = GetPrivateProfileInt("Interface","RolltheDiceButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_HeadsTailButton = GetPrivateProfileInt("Interface","HeadsTailsButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_LuckyNumberButton = GetPrivateProfileInt("Interface","LuckyNumberButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_SmithItemButton = GetPrivateProfileInt("Interface","SmithItemButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_TitleManagerButton = GetPrivateProfileInt("Interface","TitleManagerButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_PartySearchSettingButton = GetPrivateProfileInt("Interface","PartySearchSettingButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_OffAttackButton = GetPrivateProfileInt("Interface","OffAttackButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_BuyVipButton = GetPrivateProfileInt("Interface","BuyVipButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ChangeClassButton = GetPrivateProfileInt("Interface","ChangeClassButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ResetPointCheckBox = GetPrivateProfileInt("Interface","ResetPointWCCheckbox",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ResetPointCheckBox2 = GetPrivateProfileInt("Interface","ResetPointZENCheckbox",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ResetMasterSkillCheckBox = GetPrivateProfileInt("Interface","ResetMasterWCCheckbox",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ResetMasterSkillCheckBox2 = GetPrivateProfileInt("Interface","ResetMasterZENCheckBox",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_AccountSecurityButton = GetPrivateProfileInt("Interface","AccSecurityButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_JewelsBankButton = GetPrivateProfileInt("Interface","JewelsBankButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_RenameButton = GetPrivateProfileInt("Interface","RenameCharacterButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_TopPlayerButton = GetPrivateProfileInt("Interface","TopPlayerButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_PShopCurrencyButton = GetPrivateProfileInt("Interface","PShopCurrencyButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_PShopOffStoreButton = GetPrivateProfileInt("Interface","PShopOffStoreButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_RPSModeButton = GetPrivateProfileInt("Interface","RPSModeButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_NewsBoardButton = GetPrivateProfileInt("Interface","NewsBoardButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_FruitMainButton = GetPrivateProfileInt("Interface","FruitMainButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_LuckySpinButton = GetPrivateProfileInt("Interface","LuckySpinButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_GiftCodeButton = GetPrivateProfileInt("Interface","GiftCodeButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_QuickAddButton = GetPrivateProfileInt("Interface","QuickAddButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ResetStatButton = GetPrivateProfileInt("Interface","ResetPointButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_EventTimerButton = GetPrivateProfileInt("Interface","EventTimerButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_CustomWareButton = GetPrivateProfileInt("Interface","CustomWareButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_PartyLeaderButton = GetPrivateProfileInt("Interface","PartyLeaderButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_PartyTraceButton = GetPrivateProfileInt("Interface","PartyTraceButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	// Personal Shop Button
	this->m_BlessButton = GetPrivateProfileInt("Interface","BlessButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_SoulButton = GetPrivateProfileInt("Interface","SoulButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_ChaosButton = GetPrivateProfileInt("Interface","ChaosButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_wCoinCButton = GetPrivateProfileInt("Interface","wCoinCButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_wCoinPButton = GetPrivateProfileInt("Interface","wCoinPButton",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->m_GoblinPointButton = GetPrivateProfileInt("Interface","GoblinPoints",0,"..\\Data\\Custom\\ClientInterface.txt");
	// Reset Point
	this->ResetPointZEN[0]	= GetPrivateProfileInt("Interface","ResetPointZEN_Vip0",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointZEN[1]	= GetPrivateProfileInt("Interface","ResetPointZEN_Vip1",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointZEN[2]	= GetPrivateProfileInt("Interface","ResetPointZEN_Vip2",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointZEN[3]	= GetPrivateProfileInt("Interface","ResetPointZEN_Vip3",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointWC[0]	= GetPrivateProfileInt("Interface","ResetPointWC_Vip0",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointWC[1]	= GetPrivateProfileInt("Interface","ResetPointWC_Vip1",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointWC[2]	= GetPrivateProfileInt("Interface","ResetPointWC_Vip2",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetPointWC[3]	= GetPrivateProfileInt("Interface","ResetPointWC_Vip3",0,"..\\Data\\Custom\\ClientInterface.txt");
	// Reset Master Skill
	this->ResetMasterZEN[0]	= GetPrivateProfileInt("Interface","ResetMasterSkillZEN_Vip0",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterZEN[1]	= GetPrivateProfileInt("Interface","ResetMasterSkillZEN_Vip1",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterZEN[2]	= GetPrivateProfileInt("Interface","ResetMasterSkillZEN_Vip2",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterZEN[3]	= GetPrivateProfileInt("Interface","ResetMasterSkillZEN_Vip3",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterWC[0]	= GetPrivateProfileInt("Interface","ResetMasterSkillWC_Vip0",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterWC[1]	= GetPrivateProfileInt("Interface","ResetMasterSkillWC_Vip1",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterWC[2]	= GetPrivateProfileInt("Interface","ResetMasterSkillWC_Vip2",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ResetMasterWC[3]	= GetPrivateProfileInt("Interface","ResetMasterSkillWC_Vip3",0,"..\\Data\\Custom\\ClientInterface.txt");
	// Rename
	this->ChangeNameWC[0]	= GetPrivateProfileInt("Interface","RenameCharacterWC_Vip0",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ChangeNameWC[1]	= GetPrivateProfileInt("Interface","RenameCharacterWC_Vip1",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ChangeNameWC[2]	= GetPrivateProfileInt("Interface","RenameCharacterWC_Vip2",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->ChangeNameWC[3]	= GetPrivateProfileInt("Interface","RenameCharacterWC_Vip3",0,"..\\Data\\Custom\\ClientInterface.txt");
	//
	this->NewBroad_ShowOnLogin	= GetPrivateProfileInt("Interface","ShowOnLogin",0,"..\\Data\\Custom\\ClientInterface.txt");
	//
	this->TracePlayerPartyZen[0] = GetPrivateProfileInt("Interface","TraceNeedZen_Vip0",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->TracePlayerPartyZen[1] = GetPrivateProfileInt("Interface","TraceNeedZen_Vip1",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->TracePlayerPartyZen[2] = GetPrivateProfileInt("Interface","TraceNeedZen_Vip2",0,"..\\Data\\Custom\\ClientInterface.txt");
	this->TracePlayerPartyZen[3] = GetPrivateProfileInt("Interface","TraceNeedZen_Vip3",0,"..\\Data\\Custom\\ClientInterface.txt");
}

BYTE CServerInfo::INVENTORY_SIZE_SCF(int aIndex, bool Max)
{
	int S6E2 = 1;
...
BYTE CServerInfo::WAREHOUSE_SIZE_SCF(int aIndex, bool Max)
{
	int S6E2 = 1;
	if(OBJECT_RANGE(aIndex) == false)
		return WAREHOUSE_EXT1_SIZE;
