...
void CObjectManager::ObjectSetStateCreate(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpObj->RegenOk == 2)
	{
		...
		this->CharacterUpdateMapEffect(lpObj);
		/*
		gInvasionManager.CheckGoldDercon(lpObj->Map);
		gInvasionManager.CheckGreatGoldDragon(lpObj->Map);
		gInvasionManager.CheckRedDragon(lpObj->Map);
		*/
	}
}
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if(lpObj->DuelBetValue >= 1)
	{
		gCashShop.GDCashShopSubPointSaveSend(lpObj->Index,0,lpObj->DuelBetValue,0,0);
	}
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	/*else if(DE_MAP_RANGE(lpObj->Map) != 0)
	{
		result = gDiablo.GetUserRespawnLocation(lpObj,&gate,&map,&x,&y,&dir,&level);
	}*/
...
void CObjectManager::CharacterCalcExperienceParty(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	...
	if (PartyClass == 0)
	{
		if (PartyCount >= 3)
		{
			ExperienceRate = gServerInfo.m_PartySpecialExperience[(PartyCount-1)]*PartyCount;
		}
		else
		{
			ExperienceRate = gServerInfo.m_PartyGeneralExperience[(PartyCount-1)]*PartyCount;
		}
	}
	else
	{
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	{
		//lpObj->CharSet[16] |= (TempInventory[8]-146) << 1;
...
bool CObjectManager::CharacterUsePortal(LPOBJ lpObj,CItem* lpItem) // OK
{
	...
	{
		gObjMoveGate(lpObj->Index,437);
	}
	else if(lpObj->Map == MAP_ACHERON)
	{
		gObjMoveGate(lpObj->Index,443);
	}
	else if(lpObj->Map == MAP_DEVENTER)
	{
		gObjMoveGate(lpObj->Index,452);
	}
	else if(lpObj->Map == MAP_URUK)
	{
		gObjMoveGate(lpObj->Index,453);
	}
	else if(lpObj->Map == MAP_FEREA)
	{
		gObjMoveGate(lpObj->Index,454);
	}
	else
	{
		gObjMoveGate(lpObj->Index,17);
...
#else
	if(lpObj->Inventory[SourceSlot].m_Index == GET_ITEM(12,30))
	{
		return 0;
	}
#endif
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	while(change != 0)
	{
		...
		gDanhHieu.TitleOption(lpObj,1);
		//gFruitSystem.CalcAttribute(lpObj,1);
		#if(GAMESERVER_UPDATE>=701)
	...
	gDanhHieu.TitleOption(lpObj,0);
	//gFruitSystem.CalcAttribute(lpObj,0);
	#if(GAMESERVER_UPDATE>=701)
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	lpObj->IsMarried = lpMsg->IsMarried;
	memcpy(lpObj->MarryName,lpMsg->MarryName,sizeof(lpObj->MarryName));

	lpObj->iQuestIndex = lpMsg->iQuestIndex;
	lpObj->iQuestState = lpMsg->iQuestState;
	lpObj->iQuestVar = lpMsg->iQuestVar;
	lpObj->iQuestStat = lpMsg->iQuestStat;
	...
	*/
	if(lpObj->Level < 6 && gMap[lpObj->Map].CheckAttr(lpObj->X,lpObj->Y,1) == 0)
	{
		result = gGate.GetGate(gDefaultClassInfo.m_DefaultClassInfo[lpObj->Class].StartGate,&gate,&map,&x,&y,&dir,&level);
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(gObj[SummonIndex].Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
		{
			...
			/* SURVIVOR RULES START */
			if(g_Survivor.ActiveEvent > -1 && g_Survivor.State == SURVIVOR_STATE_BATTLE)
			{
				if(g_Survivor.IsInSurvivorArea(lpObj) && g_Survivor.IsInSurvivorArea(lpTarget))
				{
					//Kill count add to the killer
					lpObj->SurvivorKillCount++;
					lpTarget->SurvivorDieCount++;
					//Set survivor status = none
					if(gServerInfo.SurvivorWinnerByKillCount == 0)
					{
					lpTarget->SurvivorState = 0;
					}
					//Send messages and write log
					gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gServerInfo.SurvivorMsg[15],lpObj->Name,lpTarget->Name);
					LogAdd(LOG_BLACK,"[Survivor] [%s] killed [%s] | Kill Count: %d",lpObj->Name,lpTarget->Name,lpObj->SurvivorKillCount);
					gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gServerInfo.SurvivorMsg[16],lpObj->Name);
				}
			}
			/* SURVIVOR RULES END*/
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			gCustomQuest.MonsterKill(lpObj, lpTarget);
