...
void CMapManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.OffStoreEnable = lpMemScript->GetAsNumber();

			info.OffAttackEnable = lpMemScript->GetAsNumber();
...
int CMapManager::GetMapOffStoreEnable(int index) // OK
{
	std::map<int,MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);
	...
	{
		return ((it->second.OffStoreEnable==-1)?0:it->second.OffStoreEnable);
	}
}

int CMapManager::GetMapOffAttackEnable(int index) // OK
{
	std::map<int,MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);
	...
	{
		return ((it->second.OffAttackEnable==-1)?0:it->second.OffAttackEnable);
	}
}
