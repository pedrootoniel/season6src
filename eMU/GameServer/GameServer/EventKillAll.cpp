...
void CEventKillAll::MainProc() // OK
{
	...
	{ 
		...
		if(this->m_RemainTime <= (this->m_TotalTime-gServerInfo.m_EventKillAllTimeToEnter) && this->m_EventKillAllEnter == 1)
		{
			if (this->m_Members < 2)
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(644));
				LogAdd(LOG_BLACK,"[EVENT KILL ALL] Canceled");

				this->Clear();
			}
			else
			{
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(641));
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(642));
		...
		{
			    this->m_EventKillAllActive = 0;
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(643));
				LogAdd(LOG_BLACK,"[EVENT KILL ALL] Time is over");
			...
			for(int n=0;n < MAX_KILLALL_USER;n++)
			{
				if(OBJECT_RANGE(this->User[n].Index) != 0)
				{
					if(this->User[n].Rank == 1)
					{
						...
						{	
							//GDSetCoinSend(this->User[n].Index, gServerInfo.m_EventKillAllAutoReward1[0], gServerInfo.m_EventKillAllAutoReward2[0], gServerInfo.m_EventKillAllAutoReward3[0]);
							gCashShop.GDCashShopAddPointSaveSend(this->User[n].Index,0,gServerInfo.m_EventKillAllAutoReward1[0],gServerInfo.m_EventKillAllAutoReward2[0],gServerInfo.m_EventKillAllAutoReward3[0]);
						}

						gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(648),gObj[this->User[n].Index].Name,this->User[n].Score);

						LogAdd(LOG_BLACK,"[EVENT PVP] (Winner1: %s)",gObj[this->User[n].Index].Name);
					}
					if(this->User[n].Rank == 2)
					{
						...
						{	
							//GDSetCoinSend(this->User[n].Index, gServerInfo.m_EventKillAllAutoReward1[1], gServerInfo.m_EventKillAllAutoReward2[1], gServerInfo.m_EventKillAllAutoReward3[1]);
							gCashShop.GDCashShopAddPointSaveSend(this->User[n].Index,0,gServerInfo.m_EventKillAllAutoReward1[1],gServerInfo.m_EventKillAllAutoReward2[1],gServerInfo.m_EventKillAllAutoReward3[1]);
						}

						gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(649),gObj[this->User[n].Index].Name,this->User[n].Score);

						LogAdd(LOG_BLACK,"[EVENT PVP] (Winner2: %s)",gObj[this->User[n].Index].Name);
					}
					if(this->User[n].Rank == 3)
					{
						...
						{	
							//GDSetCoinSend(this->User[n].Index, gServerInfo.m_EventKillAllAutoReward1[2], gServerInfo.m_EventKillAllAutoReward2[2], gServerInfo.m_EventKillAllAutoReward3[2]);
							gCashShop.GDCashShopAddPointSaveSend(this->User[n].Index,0,gServerInfo.m_EventKillAllAutoReward1[2],gServerInfo.m_EventKillAllAutoReward2[2],gServerInfo.m_EventKillAllAutoReward3[2]);
						}

						gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(650),gObj[this->User[n].Index].Name,this->User[n].Score);

						LogAdd(LOG_BLACK,"[EVENT PVP] (Winner3: %s)",gObj[this->User[n].Index].Name);
					}

					gNotice.GCNoticeSend(this->User[n].Index,1,0,0,0,0,0,gMessage.GetMessage(651),this->User[n].Rank,this->User[n].Score);
					
				}

			}

			LogAdd(LOG_BLACK,"[EVENT PVP] Finish");
			this->Clear();
...
bool CEventKillAll::CommandEventKillAll(LPOBJ lpObj,char* arg) // OK 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(645),gServerInfo.m_EventKillAllMaxPlayers);
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(646));

	LogAdd(LOG_BLACK,"[EVENT KILL ALL] Start by %s",lpObj->Name);
}
...
void CEventKillAll::CommandEventKillAllJoin(LPOBJ lpObj,char* arg) // OK 
{
	...
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(647),this->m_Members, lpObj->Name);

	gObjTeleport(lpObj->Index,this->m_Map,this->m_MapX,this->m_MapY);

	LogAdd(LOG_BLACK,"[EVENT KILL ALL] Member add (%s)",lpObj->Name);
}
...
void CEventKillAll::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for(int n=0;n < MAX_KILLALL_USER;n++)
	{
		if(this->User[n].Index == lpObj->Index)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index,1,0,0,0,0,0,gMessage.GetMessage(652),this->User[n].Score);
		}
