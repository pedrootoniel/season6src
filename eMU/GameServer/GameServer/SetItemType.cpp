...
SET_ITEM_TYPE_INFO* CSetItemType::GetInfoByIndex(int index)
{
	if (this->m_SetItemTypeInfo[index].Index != -1)
	{
		return &this->m_SetItemTypeInfo[index];
	}
	return 0;
}
...
int CSetItemType::GetCountOfItems()
{
	return this->m_count;
}

int CSetItemType::GetSetOption(int itemnum)
{
	int SetOption = 0;

	for(int n=0;n < MAX_SET_ITEM_OPTION_INDEX;n++)
	{
		if(this->GetSetItemOptionIndex(itemnum,n) <= 0)
		{
			continue;
		}
