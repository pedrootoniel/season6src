...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		...
		if(gServerInfo.m_BypassShield == 1)
		{
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	float DurabilityValue2 = (1.0f/gServerInfo.m_GuardianDurabilityRate)*10000;
	DurabilityValue2 = (DurabilityValue/lpObj->GuardianDurabilityRate)*10000;
	...
	/*
	else if(lpItem->m_Index >= GET_ITEM(13,150) && lpItem->m_Index <= GET_ITEM(13,450)) // Custom Pet
	{
		lpItem->m_Durability -= (damage * 1)/1;
	}
	*/
	else
	{
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	/*
	if(gServerInfo.m_AcceptPartyKill == 0)
	{
	...
	*/
	if(gServerInfo.m_AcceptPartyKill == 0)
	{
		if(gParty.IsMember(lpObj->PartyNumber,lpTarget->Index) != 0)
		{
			return 0;
		}
	}
...
bool CAttack::MissCheckPvP(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int send,int count,BYTE* miss) // OK
{
	...
	#pragma message("PVS_STUDIO FIX 02");
	if((lpTarget->Level-lpObj->Level) >= 100 && (lpTarget->Level-lpObj->Level) < 200)
	{
		AttackSuccessRate -= 5;
	}
	else if((lpTarget->Level-lpObj->Level) >= 200 && (lpTarget->Level-lpObj->Level) < 300)
	{
...
bool CAttack::ApplySkillEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	switch(lpSkill->m_skill)
	{
		...
		case SKILL_UPPERCUT:
			...
			break;
		case SKILL_CYCLONE:
			if(gServerInfo.CyclonePushBack == 1)
			{
				gObjAddMsgSendDelay(lpTarget,2,lpObj->Index,150,0);
			}
			break;
