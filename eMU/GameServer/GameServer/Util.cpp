...
bool DataSend2(int aIndex,LPBYTE lpMsg,DWORD size) // OK
{
...
void PostMessage5(LPOBJ lpObj,char* arg) // OK
{
	char szTemp[1024];
	char szTemp2[1000];
	...
	va_list pArguments;
	va_start(pArguments, arg);
	vsprintf(szTemp, arg, pArguments);
	va_end(pArguments);

	wsprintf(szTemp2,"%s%s","[POST]",szTemp);

	PMSG_CHAT_SEND pMessage;
	memcpy(pMessage.name, lpObj->Name, 10);
	memcpy(pMessage.message, szTemp2, 90);
	pMessage.header.set(0x00,sizeof(PMSG_CHAT_SEND));
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnectedGP(n) != 0)
		{
			DataSend(n,(BYTE*)&pMessage,pMessage.header.size);
		}
	}
}
