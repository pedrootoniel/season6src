...
{
	this->Init();
	this->Read(gPath.GetFullPath("Custom\\UserDieItemDrop.xml"));
}
// -------------------------------------------------------------------------------
...
void UserDieItemDrop::Drop(LPOBJ lpUser, LPOBJ lpKiller)
{
	...
	if( lpKiller->Type == OBJECT_USER )
	{
		...
		// ----
		if( lpUser->PKLevel < this->m_PvPMinPK )
		{
			return;
		}
		// ----
	...
	// ----
	if( lpKiller->Type == OBJECT_MONSTER )
	{
		...
		// ----
		if( lpUser->PKLevel < this->m_PvMMinPK )
		{
			return;
		}
		// ----
	...
	if( this->m_MapListEnabled )
	{
		...
		{
			if( this->m_MapList[i] == lpUser->Map )
			{
				IsCorrectMap = true;
			}
		}
	...
	for( int i = 0; i < this->m_RollMaxCount; i++ )
	{
		for( int n = 0; n < INVENTORY_SIZE; n++ )
		{
			...
			// ----
			if(		lpUser->Inventory[Slot].IsItem() 
				&&	!gJewelOfHarmonyOption.IsJewelOfHarmonyItem(&lpUser->Inventory[Slot]) )
			{
				...
				lpMsg.y = lpUser->Y;
				// ----
				gItemManager.CGItemDropRecv(&lpMsg, lpUser->Index);
				LogAdd(LOG_BLACK,"[UserDieItemDrop] [%s][%s] Item has been droped (Slot: %d)",
