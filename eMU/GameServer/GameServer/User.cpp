...
void gObjCountProc() // OK
{
	...
	gObjTotalOffExp = TotalOffExp;

	gObjTotalOffTrade = TotalOffTrade;
}

void gObjAccountLevelProc() // OK
{
#if USE_OFFEXP_RELOAD == TRUE
	if (gCustomAttack.AccountsRestored == 1 && gJoinServerConnection.CheckState() != 0 && gDataServerConnection.CheckState() != 0)
	{
		gCustomAttack.RestoreOffline();
		gCustomAttack.AccountsRestored = 2;
	}

	if (gCustomAttack.AccountsRestored == 0/* && gJoinServerConnection.CheckState() != 0 && gDataServerConnection.CheckState() != 0*/)
	{
		gCustomAttack.AccountsRestored = 1;
	}
#endif
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	lpObj->DuelUser = -1;
	lpObj->DuelScore = 0;
	lpObj->DuelScore_Bet = 0;
	lpObj->DuelTickCount = 0;
	lpObj->DuelBetValue = 0;
	...
	lpObj->m_PacketCheckTime = 0;
	//Protect
	lpObj->SecurityLock = 1;
	lpObj->r00t = 0;
	//JewelsBank
	gBankEx.InitUser(lpObj);
	//BotBuffer
	lpObj->IsBot = 0;
	lpObj->BotNumOwner = -1;
	lpObj->BotPower = 0;
...
#if USE_OFFEXP_RELOAD == TRUE
	lpObj->IsOffExpReconnect = false;
#endif
	for (int i = 0; i < 4; i++)
	{
		lpObj->m_OnlineBonusTime[i] = 0;
	}
...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if (lpObj->AttackCustomOffline == 1 || lpObj->PShopCustomOffline == 1)
		{
			return -1;
		}
...
bool gObjIsAccountValidPassword(int aIndex,char* account, char* password) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
	...
	if(strcmp(lpObj->Password,password) != 0)
	{
		return 0;
	}
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gServerInfo.m_AnnouncePlayerKillPlayer == 1 && lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gServerInfo.m_AnnouncePlayerKillPlayerText,lpTarget->Name,lpObj->Name);
	}
		...
		if(lpObj->PKLevel >= 4)
		{
			...
			while(count-- != 0)
			{
				...
				if(lpObj->Inventory[number].IsItem()==1)
				{
						...
						{
							LogAdd(LOG_BLUE,"[%s][%s] PK Drop, ItemName: %s",lpObj->Account,lpObj->Name,gItemManager.GetItemName(lpObj->Inventory[number].m_Index));
							dropresult = 1;
							break;
						}
				}
	...
	//g_QuestSystem.RunHunt(lpTarget, lpObj);

	gDuel.UserDieProc(lpObj,lpTarget);

	gGensSystem.UserDieProc(lpObj,lpTarget);

	if (lpObj->HelperTotalTime != 0 || lpObj->AttackCustomOffline != 0)
	{
		SendKillMessage(lpTarget,lpObj);
	}
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	//g_UserDieItemDrop.Drop(lpTarget, lpObj);
}
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	#pragma message("Simple kalima fix (test this!!!)");
	/*
	...
	for(int n=0;n < WAREHOUSE_SIZE;n++)
	{
		if(lpObj->Warehouse[n].m_Serial != 0 && lpObj->Warehouse[n].m_Serial == serial && (count++) > 0)
		{
			...
			if(gServerInfo.DeleteDupeItem == 1)
			{
				...
				gItemManager.WarehouseItemSet(lpObj->Index,n,1);
			}
			if(gServerInfo.DisconectDupeItem == 1)
			{
				gObjUserKill(lpObj->Index);
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				//TitleDataRequest(lpObj);
				gDanhHieuDataSend(lpObj);
				...
				if(lpObj->PShopCustomOffline == 0 && lpObj->AttackCustomOffline == 0)
				{
					for (int i = 0; i < 4; i++)
					lpObj->m_OnlineBonusTime[i]++;
					gOnlineBonus.AddTimeBonus(lpObj->Index);
				}
		...
		if(lpObj->Connected > OBJECT_LOGGED &&	lpObj->Type == OBJECT_USER)
		{
			...
			if(lpObj->CrcCheckTime > 0 && GetTickCount() - lpObj->CrcCheckTime > 5000
#if USE_OFFEXP_RELOAD == TRUE
				&& lpObj->IsOffExpReconnect != true
#endif
			)
			{
				LogAdd(LOG_RED,"(ID: %s)(PLAYER: %s)(IP: %s) Invalid Game-Client", lpObj->Account, lpObj->Name, lpObj->IpAddr);
				GCCloseClientSend(n,0);
			}
			...
			if(lpObj->VaultTimeCount > 0)
				lpObj->VaultTimeCount--;
...
void gObjCustomLogPlusChaosMix(LPOBJ lpObj,int index,int level)
{
	...
	if(gServerInfo.m_AnnounceChaosMix != 0)
	{
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			wsprintf(buff,gServerInfo.m_AnnounceChaosMixText,lpObj->Name,ItemInfo.Name,level);
...
void SendKillMessage(LPOBJ lpKiller, LPOBJ lpTarget)
{
	PMSG_FRIEND_MEMO pMsg;

	pMsg.WindowGuid = 0;

	//strcpy(pMsg.Name,"AFK");
	memcpy(pMsg.Name,"AFK",sizeof(pMsg.Name));
	memcpy(pMsg.Subject,"System Message.",sizeof(pMsg.Subject));

	pMsg.Action = 143;
	pMsg.Dir = 2;

	char text[100];
	sprintf(text,"You were killed by %s",lpKiller->Name);

	memcpy(pMsg.Memo,text,sizeof(pMsg.Memo));
	pMsg.MemoSize = sizeof(text);

	FriendMessageSend(&pMsg,lpTarget->Index,lpKiller->Index);
}
...
void gObjItemTextSave(LPOBJ lpObj)
{
	...
	for ( n=0;n<gServerInfo.INVENTORY_SIZE_SCF(lpObj->Index,false);n++)
	{
		if ( lpObj->Inventory[n].IsItem() == TRUE )
		{
			if ( lpObj->Inventory[n].m_Serial != 0 )
			{
				ItemIsBufExOption(NewOption, &lpObj->Inventory[n]);
				gLog.Output(LOG_PLAYERITEM,"[%s][%s] Inventory (%d,%s,Level: %d,Skill: %d,Luck: %d,Opt: %d) Serial:(%d) Dur:(%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set: (%d)",
					lpObj->Account, lpObj->Name, n, lpObj->Inventory[n].GetName(),
					lpObj->Inventory[n].m_Level, lpObj->Inventory[n].m_Option1,
					lpObj->Inventory[n].m_Option2, lpObj->Inventory[n].m_Option3,
					lpObj->Inventory[n].m_Serial, (BYTE)lpObj->Inventory[n].m_Durability,
					NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6],
					lpObj->Inventory[n].m_SetOption);
			}
		}
	}

	gLog.Output(LOG_PLAYERITEM,"[%s][%s] Inventory Save Complete. Money ( %d )", lpObj->Account, lpObj->Name, lpObj->Money);
}
...
void gObjWarehouseTextSave(LPOBJ lpObj)
{
	...
	for ( n=0;n<gServerInfo.WAREHOUSE_SIZE_SCF(lpObj->Index,false);n++)
	{
		if ( lpObj->Warehouse[n].IsItem() == TRUE )
		{
			if ( lpObj->Warehouse[n].m_Serial != 0 )
			{
				ItemIsBufExOption(NewOption, &lpObj->Warehouse[n]);

				gLog.Output(LOG_PLAYERITEM,"[%s][%s] Warehouse (%d,%s,Level: %d,Skill: %d,Luck: %d,Opt: %d) Serial:(%d) Dur:(%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set: (%d)",
					lpObj->Account, lpObj->Name, n, lpObj->Warehouse[n].GetName(),
					lpObj->Warehouse[n].m_Level, lpObj->Warehouse[n].m_Option1,
					lpObj->Warehouse[n].m_Option2, lpObj->Warehouse[n].m_Option3,
					lpObj->Warehouse[n].m_Serial, (BYTE)lpObj->Warehouse[n].m_Durability,
					NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6],
					lpObj->Warehouse[n].m_SetOption);
			}
		}
	}

	gLog.Output(LOG_PLAYERITEM,"[%s][%s] Warehouse Save Complete. Money(%d)", lpObj->Account, lpObj->Name, lpObj->WarehouseMoney);
}
/*
// -- BEGIN CHAOS BOX AUTO --
BYTE gObjTempGetChaosBoxSlot(LPOBJ lpObj, BYTE source)
{
	int iwidth, iheight;
	BYTE blank = 0;

	int chaoBoxSlot[4][8] = {0};
	int item_data_x, item_data_y;
	for (int y = 0; y < 4; y ++) {
		for (int x = 0; x < 8; x ++) {
			if(lpObj->ChaosBox[y*8 + x].IsItem()) {
				lpObj->ChaosBox[y*8 + x].GetSize(item_data_x, item_data_y);

				for (int y_status = y; y_status < (y + item_data_y); y_status ++) {
					for (int x_status = x; x_status < (x + item_data_x); x_status ++) {
						chaoBoxSlot[y_status][x_status] = 1;
					}
				}
			}
		}
	...
	lpObj->Inventory[source].GetSize(iwidth, iheight);
	//LogAddC(4,"W H %d %d", iwidth, iheight);

	for (int y = 0; y <= 4 - iheight; y ++) {
		for (int x = 0; x <= 8 - iwidth; x ++) {
			o_accept_flag = 1;
			// Kiem tra nhung o lien quan
			//if (lpObj->pChaosBox[y*8 + x].IsItem() == 0)
			//{
				for (int y_check = y; y_check < y + iheight; y_check ++) {
					for (int x_check = x; x_check < x + iwidth; x_check ++) {
						if (chaoBoxSlot[y_check][x_check] == 1) {
							o_accept_flag = 0;
							break;
						}
					}
	...
	if (o_accept_x != -1 && o_accept_y != -1) {
		slot_accept = o_accept_y * 8 + o_accept_x;
	} else {
		slot_accept = -1;
	}
