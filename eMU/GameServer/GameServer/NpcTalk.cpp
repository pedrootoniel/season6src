...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if( gCustomQuest.Dialog(lpObj, lpNpc) )
	{
		return 1;
	}
	...
	if (g_PersonalShopEx.NPC_Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	switch(lpNpc->Class)
...
{
	gGoldenArcher.GoldenArcherNPC(lpNpc, lpObj);
}
...
{
	if(gServerInfo.m_CustomWareButton == 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(603));
		}
	}
	else
	{
		if(lpObj->VaultTimeCount == 0)
		{
			...
			pMsg.header.set(0xFB,0x27, sizeof(pMsg));
