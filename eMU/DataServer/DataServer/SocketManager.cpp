...
bool CSocketManager::DataSend2(int index, unsigned char* lpMsg, DWORD dwSize) // OK
{
	this->m_critical.lock();
	...
	if(dwSize > MAX_MAIN_PACKET_SIZE)
	{
		LogAdd(LOG_RED,"[SocketManager] Max msg size (Type: 1, Index: %d, Size: %d)",index,dwSize);
		this->m_critical.unlock();
		return 0;
	}
	...
	if(lpIoContext->IoSize > 0)
	{
		if((lpIoContext->IoSideBuffer.size+dwSize) > MAX_SIDE_PACKET_SIZE)
		{
			LogAdd(LOG_RED,"[SocketManager] Max msg size (Type: 2, Index: %d, Size: %d)",index,(lpIoContext->IoSideBuffer.size+dwSize));
			this->Disconnect(index);
			this->m_critical.unlock();
			return 0;
		}

		memcpy(&lpIoContext->IoSideBuffer.buff[lpIoContext->IoSideBuffer.size],lpMsg,dwSize);
		lpIoContext->IoSideBuffer.size += dwSize;
		this->m_critical.unlock();
		return 1;
	}

	memcpy(lpIoContext->IoMainBuffer.buff,lpMsg,dwSize);

	lpIoContext->wsabuf.buf = (char*)lpIoContext->IoMainBuffer.buff;

	lpIoContext->wsabuf.len = dwSize;

	lpIoContext->IoType = IO_SEND;

	lpIoContext->IoSize = dwSize;
...
void CSocketManager::DataSendAll(unsigned char* lpMsg, DWORD dwSize)
{
	for(int i=0; i<this->MaxObj ;i++)
	{
		if(this->sObj[i].Connected == true)
		{
			this->DataSend2(i,lpMsg,dwSize);
		}
	}
}