...
void CWinConsole::CreateDbgFile(void)
{
	...
	//if ( !CWinConsole::g_DbgFp )
		//g_Console.AddMessage(5,"LuaDbgLog file create error");
...
HWND CWinConsole::StartConsole(HINSTANCE hInstance, lua_State* pScriptContext)
{
	...
	if ( !g_WinConsole )
	{
		g_WinConsole = new CWinConsole();
		CWinConsole::CreateDbgFile();
	}

	if ( !CWinConsole::m_hWnd )
		g_WinConsole->Init(hInstance);

	if ( pScriptContext )
	{
		g_WinConsole->m_pScriptContext = pScriptContext;
...
{
	g_WinConsole->SaveDbgWnd( luaL_checklstring(L, 1, 0) );
...
LRESULT __stdcall CWinConsole::MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
			...
			switch(LOWORD(wParam))
			{
				...
				case IDM_WINCONS_SAVE:
				{
					...
					if ( GetSaveFileName(&ofn) != 0 )
						g_WinConsole->SaveDbgWnd(szFullPath);
				}
				break;
				...
				case IDM_WINCONS_DEBUG:
					...
					{
						CheckMenuItem(g_WinConsole->GetSubMenu(), IDM_WINCONS_DEBUG, MF_UNCHECKED);
						g_WinConsole->SubtractWinConsoleOption(2);
					}
					else
					{
						CheckMenuItem(g_WinConsole->GetSubMenu(), IDM_WINCONS_DEBUG, MF_CHECKED);
						g_WinConsole->AddWinConsoleOption(2);
					}
				break;
				...
				case IDM_WINCONS_TOPMOST:
					...
					{
						CheckMenuItem(g_WinConsole->GetSubMenu(), IDM_WINCONS_TOPMOST, MF_UNCHECKED);
						g_WinConsole->SubtractWinConsoleOption(4); // bug? maybe 4
					...
					{
						CheckMenuItem(g_WinConsole->GetSubMenu(), IDM_WINCONS_TOPMOST, MF_CHECKED);
						g_WinConsole->AddWinConsoleOption(4);
		...
		case 1028:	// 0x404
			...
			if ( g_WinConsole->m_pScriptContext )
			{
				if ( luaL_loadbuffer(g_WinConsole->m_pScriptContext, CWinConsole::m_CommandBuffer, strlen(CWinConsole::m_CommandBuffer), NULL) )
					CWinConsole::Write("Error loading Command\n");

				if ( lua_pcall(g_WinConsole->m_pScriptContext, 0, -1, 0) )
				{
					CWinConsole::Write("Error in Command\n");
					CWinConsole::Write(luaL_checklstring(g_WinConsole->m_pScriptContext, -1, NULL));
				}
			}
...
void CWinConsole::Write(char const * pString, ...)
{
	...
	if ( pString && g_WinConsole && CWinConsole::m_hWnd )
	{
		...
		if(indx > 0)
		{
			...
			LRESULT lResult = SendMessageTimeoutA(CWinConsole::m_hListBox, LB_ADDSTRING,
...
void CWinConsole::ClearDbgWnd(void)
{
	...
	if ( g_WinConsole && CWinConsole::m_hWnd)
	{
		iTimeOut = 1;
		lpdwResult = 0;
...
bool CWinConsole::EnableWinConsoleOption(DWORD dwWinConsoleOpt)
{
	if(g_WinConsole && CWinConsole::m_hWnd)
	{
		if( (g_WinConsole->GetWinConsoleOpt() & dwWinConsoleOpt) )
			return true;
	}
