...
bool MULua::Generic_Call(const char* func, const char* sig, ...)
{
	...
	while (*sig)
	{
		...
		default:
		{
			//g_Console.AddMessage(5,"luacall_Generic_Call invalid option (%c)", *(sig - 1));
	...
	{
		//g_Console.AddMessage(5,"luacall_Generic_Call error running function '%s': '%s'", func, lua_tolstring(this->m_luaState, -1, 0));
