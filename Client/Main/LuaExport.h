...
public:
	...
	static BOOL LuaSetBkColor(lua_State * Lua/*BYTE red, BYTE green, BYTE blue, BYTE alpha*/)
	{
		pSetBkColor(pTextThis(),(BYTE)lua_tointeger(Lua,1),(BYTE)lua_tointeger(Lua,2),(BYTE)lua_tointeger(Lua,3),(BYTE)lua_tointeger(Lua,4));
		return true;
	}

	static BOOL LuaSetTextColor(lua_State * Lua/*BYTE red, BYTE green, BYTE blue, BYTE alpha*/)
	{
		pSetTextColor(pTextThis(),(BYTE)lua_tointeger(Lua,1),(BYTE)lua_tointeger(Lua,2),(BYTE)lua_tointeger(Lua,3),(BYTE)lua_tointeger(Lua,4));
		return true;
	}

	static BOOL LuaSetFont(lua_State * Lua/*int a2*/)
	{
		pSetFont(pTextThis(),(int)*(HGDIOBJ*)lua_touserdata(Lua,1));
		return true;
	}
