...
int WideScreen = GetPrivateProfileInt("WideScreen","WideScreen",0,".\\Settings.ini");
...
__declspec(naked) void ResolutionSwitch() // OK
{
	...
	if(WideScreen == 1)
	{
		x = GetPrivateProfileInt("WideScreen","X",980,".\\Settings.ini");
		y = GetPrivateProfileInt("WideScreen","Y",600,".\\Settings.ini");
	}
	else
	{
		x = 800;
	...
	_asm
	{
		...
		Jnz NEXT2
		Mov Eax,x
		Mov Dword Ptr Ds:[MAIN_RESOLUTION_X],Eax
		Mov Ecx,y
		Mov Dword Ptr Ds:[MAIN_RESOLUTION_Y],Ecx
		Jmp EXIT
		NEXT2:
...
//		Cmp Dword Ptr Ss:[Ebp-0x08],0x300
//		Jbe EXIT
