...
void cMiniMap::InsertPartyInfo(MINIMAP_PARTY_INFO* lpInfo)
{
	...
	//g_Console.AddMessage(5,"%s %d %d %d",this->gPartyInfo[this->ListsCount].name,this->gPartyInfo[this->ListsCount].map = lpInfo->map,this->gPartyInfo[this->ListsCount].x = lpInfo->x,this->gPartyInfo[this->ListsCount].y = lpInfo->y);
	this->ListsCount++;
}