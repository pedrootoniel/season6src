...
void InitReconnect() // OK
{
	...
	SetCompleteHook(0xE9,0x0040B903,&ReconnectGetAccountInfo);

	SetCompleteHook(0xE9,0x004DA416,&ReconnectCheckConnection);

	SetCompleteHook(0xE9,0x0063A192,&ReconnectCloseSocket);

	SetCompleteHook(0xE9,0x007A89EB,&ReconnectMenuExitGame);

	SetCompleteHook(0xE8,0x0040BC24,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x004D762C,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x004EF622,&ReconnectCreateConnection);

	SetCompleteHook(0xE8,0x0063CD3A,&ReconnectCreateConnection);

	//SetCompleteHook(0xE8,0x0080F7FE,&ReconnectMainProc);
}

void ReconnectMainProc() // OK
{
	//((void(*)())0x0080F8E0)(); //already called in Interface.cpp
...
void ReconnecGameServerAuth() // OK
{
	...
	pMsg.ClientVersion[0] = (*(BYTE*)(0x00E61F68))-1;

	pMsg.ClientVersion[1] = (*(BYTE*)(0x00E61F69))-2;

	pMsg.ClientVersion[2] = (*(BYTE*)(0x00E61F6A))-3;

	pMsg.ClientVersion[3] = (*(BYTE*)(0x00E61F6B))-4;

	pMsg.ClientVersion[4] = (*(BYTE*)(0x00E61F6C))-5;

	memcpy(pMsg.ClientSerial,(void*)0x00E61F70,sizeof(pMsg.ClientSerial));
...
void ReconnectViewportDestroy() // OK
{
	...
	do
	{
		...
		{
			if((ViewportAddress=((DWORD(__thiscall*)(void*,DWORD))0x0096A4C0)(((void*(*)())0x00402BC0)(),count)) == 0)
			{
				continue;
			}
...
BOOL ReconnectCreateConnection(char* address,WORD port) // OK
{
	...
	return ((BOOL(*)(char*,WORD))0x0063C960)(address,port);
}

__declspec(naked) void ReconnectGetAccountInfo() // OK
{
	static DWORD ReconnectGetAccountInfoAddress1 = 0x0040B90D;
	...
	_asm
	{
		...
		Call [MemoryCpy]
		Add Esp,0x0C
		Mov Dword Ptr Ds:[0x08793714],0x01
...
{
	static DWORD ReconnectCheckConnectionAddress1 = 0x004DA41B;
	static DWORD ReconnectCheckConnectionAddress2 = 0x004DA491;
...
{
	static DWORD ReconnectCloseSocketAddress1 = 0x0063A19C;
...
{
	static DWORD ReconnectMenuExitGameAddress1 = 0x007A89F0;
