...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		if(gAntiFlood.Check(inet_ntoa(SocketAddr.sin_addr)) == false)
		{
			closesocket(socket);
			lpSocketManager->m_critical.unlock();
			continue;
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		//LogAdd(LOG_RED,"getLastError = %d",GetLastError());
		...
		{
			case IO_RECV:
				//LogAdd(LOG_RED,"Size = %d - %d",IoSize,sizeof(lpIoContext->wsabuf));
