...
public:
	...
	static void GetSerial(int ver1)
	{
		...
		if( code != 0 )
		{
			wsprintf(temptext,"License server access error: %d",code);
			MessageBox(0,temptext,"WWW.MUTEAM.XYZ",0);
			CProtect::CloseProcess();
		}
		...
		{
			wsprintf(temptext, "License server access error: %s", e.what() );
			MessageBox(0,temptext,"WWW.MUTEAM.XYZ",0);
			CProtect::CloseProcess();
		}
		...
		{
			MessageBox(0,"Authentication failed!\nCheck the data in the license panel, or contact us.","WWW.MUTEAM.XYZ",MB_OK|MB_ICONERROR);
			CProtect::CloseProcess();
		}
		else
		{
	...
	static void GetLicense(const char* body)
	{
		...
		happyhttp::Connection conn( "4pinoy.muteam.xyz", 80 );
