...
void Connection::connect()
{
	...
	if( !addr )
		throw Wobbly( "Endereço de rede inválido. Verifique a conexão do servidor, incluindo servidores DNS." );
				...
				m_State == CHUNKEND )
		{
			...
			while( count > 0 )
			{
				...
				if( c == '\n' )
				{
					...
					m_LineBuf.erase(m_LineBuf.begin(),m_LineBuf.end());
...
void Response::ProcessChunkLenLine( std::string const& line )
{
	...
	if( m_ChunkLeft == 0 )
	{
		...
		m_State = TRAILERS;
		m_HeaderAccum.erase(m_HeaderAccum.begin(),m_HeaderAccum.end());
	}
	else
	{
		m_State = BODY;
